...
extern kal_uint32 custom_get_mccmnc_from_imsi(kal_uint8* imsi, kal_uint8 mnc_length);
...
reconfigure_module_info_struct reconfig_mod_info_tbl[] = 
                    {
                        ...
                        {MOD_IMC,    L4C_ME_SAP,     RECONFIG_SIM_SWITCH | RECONFIG_SBP_CHANGE,  CFG_GEMINI_MOD, NULL},
                        {MOD_VDM,    L4C_ME_SAP,     RECONFIG_SBP_CHANGE,  CFG_GEMINI_MOD, NULL},
                        {MOD_LTECSR, L4C_ME_SAP,     RECONFIG_SBP_CHANGE,  CFG_GEMINI_MOD, NULL},
#else
                        {MOD_IMC,    L4C_ME_SAP,     RECONFIG_SBP_CHANGE,  CFG_SINGLE_MOD, NULL},
                        {MOD_VDM,    L4C_ME_SAP,     RECONFIG_SBP_CHANGE,  CFG_SINGLE_MOD, NULL},
                        {MOD_LTECSR, L4C_ME_SAP,     RECONFIG_SBP_CHANGE,  CFG_SINGLE_MOD, NULL},
#endif /* __MULTIPLE_IMS_SUPPORT__ */
#endif
                        {MOD_CC,     L4C_ME_SAP,     RECONFIG_SBP_CHANGE,  CFG_GEMINI_MOD, NULL},
                        {MOD_IWLAN,  L4C_ME_SAP,     RECONFIG_SBP_CHANGE,  CFG_GEMINI_MOD, NULL},
#ifdef __GEMINI_LTE__
                        {MOD_MBMSAL, L4C_ME_SAP,     RECONFIG_SIM_SWITCH,  CFG_GEMINI_MOD, NULL},
                        {MOD_EVAL,   L4C_ME_SAP,     RECONFIG_SIM_SWITCH,  CFG_GEMINI_MOD, NULL},
#else
                        {MOD_MBMSAL, L4C_ME_SAP,     RECONFIG_SIM_SWITCH,  CFG_SINGLE_MOD, NULL},
                        {MOD_EVAL,   L4C_ME_SAP,     RECONFIG_SIM_SWITCH,  CFG_SINGLE_MOD, NULL},
#endif
                        {MOD_SMU,    L4C_ME_SAP,     RECONFIG_SIM_SWITCH,  CFG_SINGLE_MOD, NULL},
...
reconfigure_module_info_struct reconfig_mod_info_tbl[] = 
                    {
                        ...
                        {MOD_IMC,    L4C_ME_SAP,     RECONFIG_SIM_SWITCH | RECONFIG_SBP_CHANGE | RECONFIG_IMS_CHANGE,  CFG_GEMINI_MOD, nvram_custom_config_ims_profile},
                        ...
                        {MOD_SDM,    L4C_ME_SAP,     RECONFIG_IMS_CHANGE,  CFG_SINGLE_MOD, nvram_custom_config_sdm_ads_profile},
...
#endif /* __MULTIPLE_IMS_SUPPORT__ */
#endif
                        {MOD_IWLAN,  L4C_ME_SAP,     RECONFIG_SBP_CHANGE | RECONFIG_IMS_CHANGE,  CFG_SINGLE_MOD, nvram_custom_config_iwlan_profile},
                        {MOD_CC,     L4C_ME_SAP,     RECONFIG_SBP_CHANGE,  CFG_GEMINI_MOD, NULL},
                        {MOD_SMU,    L4C_ME_SAP,     RECONFIG_SIM_SWITCH,  CFG_SINGLE_MOD, NULL},
                        {MOD_L4BSAT, L4C_ME_SAP,     RECONFIG_SIM_SWITCH,  CFG_GEMINI_MOD, NULL},
                        {MOD_L4C,    L4C_ME_SAP,     RECONFIG_SIM_SWITCH,  CFG_GEMINI_MOD, NULL},
...
void custom_config_roaming_vdp_by_sim(kal_uint8* imsi, kal_uint8 mnc_length, kal_uint8 *custom_vdp_setting)
{
    kal_uint32 mccmnc;
    kal_uint32 i;
    ...
    /* Get mccmnc */
    mccmnc = custom_get_mccmnc_from_imsi(imsi, mnc_length);
...
typedef struct {    
    rac_plmn_table imsi;
	sbp_md_feature_enum sbp; //sbp enum
    kal_bool is_enable_sbp; //this sbp shall be enable or not
} sbp_imsi_table;

//For now, only SBP_RAC_CHANGE_VDP_WHEN_IMS_ONOFF support this custom table, if you want do customization for other SBP, please contact RAC owner
const static sbp_imsi_table s_custom_sbp_table[] = 
{
	/*if SIM is still not ready, the IMSI remain default value which means all zero.
      If we calculate mcc/mnc from all zero IMSI, the result is also zero and match our custom table default entry which shall not be match by any input mcc/mnc
      That is why We change the default entry mcc/mnc to 0xFEFEFEFE which will not be match by any input imsi include default value. 
    */
    {{0xFEFEFEFE, 0xFEFEFEFE}, 0, KAL_FALSE}, // default never match
	//{{302720, 302720},SBP_RAC_CHANGE_VDP_WHEN_IMS_ONOFF, KAL_FALSE},
};

/*****************************************************************************
* FUNCTION
*  	custom_config_sbp_by_sim()
* DESCRIPTION
*   only SBP_RAC_CHANGE_VDP_WHEN_IMS_ONOFF support customization
*   check the custom sbp table by imsi.
*   if both plmn and sbp hit the entry in table, return true and update is_enable flag
*   if not-hit, return false and sbp remain original setting  
*  
* PARAMETERS
*	kal_uint8* imsi
...
*
* RETURNS
*	kal_bool KAL_TRUE for table entry match
*            KAL_FALSE for mismatch
*****************************************************************************/
kal_bool custom_config_sbp_by_sim(kal_uint8* imsi, kal_uint8 mnc_length, kal_bool *is_enable, sbp_md_feature_enum feature, protocol_id_enum ps_id)
{
    kal_uint32 mccmnc;
    kal_uint32 i;
    kal_uint32 table_size = sizeof(s_custom_sbp_table) / sizeof(s_custom_sbp_table[0]);

    /* Get mccmnc */
    mccmnc = custom_get_mccmnc_from_imsi(imsi, mnc_length);
    
    /* Scan the s_sdp_imsi_table table*/
    for(i = 0; i < table_size; i++)
    {        
        if ((s_custom_sbp_table[i].imsi.mccMnc_range_start <= mccmnc) && 
            (s_custom_sbp_table[i].imsi.mccMnc_range_end >= mccmnc))
        {
            // Found in the table and sbp match
			if(s_custom_sbp_table[i].sbp == feature)
			{
                *is_enable = s_custom_sbp_table[i].is_enable_sbp;
				return KAL_TRUE;
			}
        }
    }

    /* NOT Found in the table, then return false for remaining original sbp setting */
    return KAL_FALSE;
}

#ifdef __IMS_SUPPORT__ 
typedef struct {
    rac_plmn_table plmn_range;
    kal_bool  is_smsoip_support;
} custom_config_smsoip_tbl_struct;

const static custom_config_smsoip_tbl_struct plmns_to_customize_smsoip[] = 
{
    {{71606, 71606}, KAL_FALSE},   // Movistar-Peru
    {{716006, 716006}, KAL_FALSE}, // Movistar-Peru
    {{732123, 732123}, KAL_FALSE}, // Movistar-columbia
    {{72207, 72207}, KAL_FALSE},   // Movistar - Argentina
    {{72406, 72406}, KAL_FALSE},   // Vivo-Brazil
    {{72410, 72411}, KAL_FALSE},   // Vivo-Brazil
    {{72423, 72423}, KAL_FALSE},   // Vivo-Brazil
    {{334030, 334030}, KAL_FALSE}, // Movistar-Maxico
    {{33403, 33403}, KAL_FALSE},   // Movistar-Maxico
    {{73002, 73002}, KAL_FALSE},   // Chile
    {{74000, 74000}, KAL_FALSE},   // Movistar-Ecuador
    {{74807, 74807}, KAL_FALSE},   // Movistar-Uruguay
    {{73404, 73404}, KAL_FALSE},   // Movistar-Venezuela
};

/*****************************************************************************
* FUNCTION
*  	custom_is_config_smsoip_by_sim()
* DESCRIPTION
*   This custom function is to customize the SMS over IP settings.
*   It will check the plmns_to_customize_smsoip table by imsi.
*   If plmn hit the entry in table, return true and update smsoip flag.
*   If not-hit, return false and smsoip remain original setting.
*  
* PARAMETERS
*	kal_uint8* imsi        [IN] IMSI
*   kal_uint8 mnc_length   [IN] The length of MNC (2 digits or 3 digits)
*   kal_bool* smsoip       [OUT] the customs SMS over IP settings if PLMN found in plmns_to_customize_smsoip table
*
* RETURNS
*	kal_bool KAL_TRUE for table entry match
*            KAL_FALSE for mismatch
*****************************************************************************/
kal_bool custom_is_config_smsoip_by_sim(kal_uint8* imsi, kal_uint8 mnc_length, kal_bool* smsoip)
{
    kal_uint32 mccmnc;
    kal_uint32 i;
    kal_uint32 table_size = sizeof(plmns_to_customize_smsoip) / sizeof(plmns_to_customize_smsoip[0]);
	...
	for (i = 0; i < table_size; i++)
	{
		if ((plmns_to_customize_smsoip[i].plmn_range.mccMnc_range_start <= mccmnc) && 
            (plmns_to_customize_smsoip[i].plmn_range.mccMnc_range_end >= mccmnc)) //plmn found
		{
			*smsoip = plmns_to_customize_smsoip[i].is_smsoip_support;
			return KAL_TRUE;
		}
	}
...
/*****************************************************************************
* FUNCTION
*  	custom_get_mccmnc_from_imsi()
* DESCRIPTION
*   This custom function is to decode MccMnc from IMSI.
*  
* PARAMETERS
*	kal_uint8* imsi        [IN] IMSI
*   kal_uint8 mnc_length   [IN] The length of MNC (2 digits or 3 digits)
...
*	kal_uint32 MccMnc
*****************************************************************************/
kal_uint32 custom_get_mccmnc_from_imsi(kal_uint8* imsi, kal_uint8 mnc_length)
{
    kal_uint32 mccmnc;
