 ...
 * removed!
 *
>>>> ORIGINAL //UMOLYA/BRANCHES_LR12A/LR12A.R2.MP/mcu/pcore/custom/modem/common/ps/custom_imc_config.c#258
==== THEIRS //UMOLYA/BRANCHES_LR12A/LR12A.R2.MP/mcu/pcore/custom/modem/common/ps/custom_imc_config.c#259
 * removed!
 * removed!
 * removed!
 *
 ...
 * removed!
 *
==== YOURS //ws_hanyan.zhang_nvram_check_in/UMOLYA/BRANCHES_LR12A/LR12A.R2.MP/mcu/pcore/custom/modem/common/ps/custom_imc_config.c
 * removed!
 * removed!
 *
 * removed!
...
static kal_bool set_conf_factory_uri_by_imsi(kal_uint8 *conf_factory_uri, kal_bool with_imsi_info, kal_uint8 imsi_mnc_len, kal_uint8 *mccmnc, kal_uint32 op_id)
{
    ...
    switch(op_id)
    {
        ...
        case 6: /* VDF */
        {
            ...
            /* Default (Spain/Germany/Portugal/Romania/UK/Turkey/Czech/Zambia/Hungary/Romania...) */
            else {
                if(imsi_mnc_len == 2) {
                    kal_snprintf((char *)conf_factory_uri, len-1, "sip:mmtel@conf-factory.ims.mnc0%c%c.mcc%c%c%c.3gppnetwork.org", mccmnc[3], mccmnc[4], mccmnc[0], mccmnc[1], mccmnc[2]);
                }
...
void imc_set_mtk_default_value(nvram_ef_ims_profile_record_struct *nvram_ims_profile_ptr)
{
    ...
    kal_snprintf((char *)ua->last_known_cell_id, 64-1, "0");
    kal_snprintf((char *)ua->last_known_cell_timestamp, 32-1, "0");
...
nvram_ef_ims_profile_record_struct* imc_nvram_customization(kal_uint16 op_id, sim_interface_enum sim_slot_id, kal_bool with_imsi_info, kal_uint8 imsi_mnc_len, kal_uint8 *mccmnc)
{
    ...
    switch(op_id)
    {
        ...
        case 8: /* TMOUS */
        {
            ...
            nvram_ims_profile_ptr->imc_config.rfc5626_flow_recovery  = 1;
            nvram_ims_profile_ptr->imc_config.allow_emergency_dereg  = 1;   // deactivate ECC PDN directly
        ...
        case 12: /* VzW */
        {
            ...
            nvram_ims_profile_ptr->imc_config.allow_emergency_dereg    = 2;           //VZ_REQ_e911_30547 deactivate emergency PDN directly
        ...
        case 17: /* DoCoMo */
        {
            ...
            nvram_ims_profile_ptr->ua_config.call_tcall_timer_timeout       = 32;
            nvram_ims_profile_ptr->ua_config.transfer_conf_call_as_1to1     = 0;
            nvram_ims_profile_ptr->ua_config.restore_in_3gpp_abnormal       = 2;
            nvram_ims_profile_ptr->ua_config.when_stop_tcall                = 100;
#ifdef __EVS_SUPPORT__
            nvram_ims_profile_ptr->ua_config.evs_support                    = 1;
            nvram_ims_profile_ptr->ua_config.UA_call_codec_order1           = 17;
        ...
        case 20: /* Sprint */
        {
            ...
            /* Reg/Stack */
            nvram_ims_profile_ptr->ua_config.register_expiry                = 3600;
            nvram_ims_profile_ptr->ua_config.add_timezone_to_pani           = 3;
        ...
        case 188: /* A1 */
        {
            ...
            if(strncmp((char *)&mccmnc[0], "232", 3) == 0){                
                nvram_ims_profile_ptr->imc_config.ims_reg_allowed_at_23g            = 1;        // IMS registration allowed and maintained in 23G as well
                nvram_ims_profile_ptr->imc_config.nw_vops_rule                      = 3;        // 11000, IMS reg maintained but initial reg not allowed
                nvram_ims_profile_ptr->imc_config.no_ims_reg_during_active_cs_call  = 1;        // not register for WFC during a CS call
        ...
        case 215: /* Cellcard/Cambodia */
        {
            /* IMCB, Reg/Stack, UA internal configurations */
            /* Call */
            nvram_ims_profile_ptr->ua_config.operator_code                      = 0x00D7;   //operator_code = 215 (Cellcard Cambodia)
            nvram_ims_profile_ptr->ua_config.call_ringing_timer_timeout           = 60;
            nvram_ims_profile_ptr->ua_config.call_ringback_timer_timeout          = 20;
            nvram_ims_profile_ptr->ua_config.VoLTE_Setting_SIP_TCP_MTU_Size       = 1300;
        ...
        case 235: /* VIP */
        {
            ...
            if(with_imsi_info){
                ...
                if(strncmp((char *)&mccmnc[0], "220", 3) == 0){
                    nvram_ims_profile_ptr->ua_config.operator_code                      = 0x00eb;     //operator_code = 235(VIP Serbia)
                    nvram_ims_profile_ptr->imc_config.nw_vops_rule                      = 3;          // 11000, IMS reg maintained but initial reg not allowed
                    nvram_ims_profile_ptr->imc_config.no_ims_reg_during_active_cs_call  = 1;          // not register for WFC during a CS call
                    nvram_ims_profile_ptr->imc_config.srvcc_no_ims_dereg_until_call_rel = 1;          // delay IMS dereg in 23G until call is released
        ...
        case 279: /*DITO Philippines */
        {
            ...
            set_conf_factory_uri_by_imsi(&nvram_ims_profile_ptr->ua_config.UA_conf_factory_uri[0], with_imsi_info, imsi_mnc_len, mccmnc, op_id);
            nvram_ims_profile_ptr->ua_config.VoLTE_Setting_SIP_Force_Use_UDP = 1;
            nvram_ims_profile_ptr->imc_config.resouce_retain_timer              = 5000;     //resource_retain_timer = 4000ms (4 seconds)
            nvram_ims_profile_ptr->imc_config.sms_support                   = 0;
