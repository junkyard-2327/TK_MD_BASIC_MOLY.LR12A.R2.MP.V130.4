...
/****************************************************************************
* MTK default value of Count for CS identity procedure is 2. 
* When MM_CUSTOM_CS_REG_IDENTITY_COUNT is 0, MTK shall use Count for CS identity procedure to be 2.
* Example: say if Count for CS identity procedure needs to be customized to 5 then set MM_CUSTOM_CS_REG_IDENTITY_COUNT = 5.
****************************************************************************/
const kal_uint8 MM_CUSTOM_CS_REG_IDENTITY_COUNT = 2;
/****************************************************************************
* MTK default value of Count for PS identity procedure is 2. 
* When GMM_CUSTOM_PS_REG_IDENTITY_COUNT is 0, MTK shall use Count for PS identity procedure to be 2.
* Example: say if Count for PS identity procedure needs to be customized to 5 then set GMM_CUSTOM_PS_REG_IDENTITY_COUNT = 5.
****************************************************************************/
const kal_uint8 GMM_CUSTOM_PS_REG_IDENTITY_COUNT = 2;
...
const char *CUSTOM_IRAT_TAU_TYPE_FOR_LAZY_GMM[] = {"46697F","42902F","45008F","41004F","63903F","52503F","46060F","52047F","50218F","29701F","52501F","46001F","74000F",INVALID_CUSTOM_PLMN};
...
*
*   - If PLMN is listed in the array, then UE shall not set FOR bit in GMM ATTACH Request
...
*
*****************************************************************************/
kal_uint8 mm_custom_get_customize_cause(protocol_id_enum ps_id,kal_uint8 cause, mm_custom_proc_type proc, kal_bool is_on_hplmn, kal_bool is_test, kal_bool convert_vplmn_ps_cause7to15)
{
...
kal_uint8 mm_custom_get_proc_cause_converter(kal_uint8 cause, mm_custom_proc_msg cust_proc, kal_bool is_on_hplmn, kal_bool is_test, protocol_id_enum protocol_id)
{
    ...
    case CUST_MM_MSG_LU_REJECT:
        ...
        case 0x0c:
             /* Customer Req is to convert cause #13 to cause #15, irrspective of HPLMN or VPLMN 
			    Removing the below check of HPLMN & directly return cause#15			  
            if (is_on_hplmn == KAL_TRUE)
            {
                return 0x0f;
            }
...
/*****************************************************************************
* FUNCTION
*  mm_custom_cs_reg_identity_count_value
* DESCRIPTION
*  This function is to get Identity Count for CS procedure set by customer 
*  
* PARAMETERS
*  None
*
...
*
*****************************************************************************/
kal_uint8 mm_custom_get_cs_reg_identity_count_value(void)
{
    return MM_CUSTOM_CS_REG_IDENTITY_COUNT;
}
/*****************************************************************************
* FUNCTION
*  gmm_custom_ps_reg_identity_count_value
* DESCRIPTION
*  This function is to get Identity Count for PS procedure set by customer 
*  
* PARAMETERS
*  None
*
...
*
*****************************************************************************/
kal_uint8 gmm_custom_get_ps_reg_identity_count_value(void)
{
    return GMM_CUSTOM_PS_REG_IDENTITY_COUNT;
}
