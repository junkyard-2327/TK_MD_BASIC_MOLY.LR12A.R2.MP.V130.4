...
sub scatGen_main
{
    ...
    if($compiler ne "RVCT")
    {
        &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Unsupported Compiler: $compiler !", __FILE__, __LINE__);
    }
    ...
    if(&isNAND($func))
    {
        if ($bb eq 'MT6253')
        {
            &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Unsupported Feature Combination NFB on platform $bb!", __FILE__, __LINE__);
        }
          
        if (defined $zimage and $zimage eq 'TRUE')
        {
            &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Unsupported Feature Combination : ZIMAGE + NFB!", __FILE__, __LINE__);
        }
    ...
    else
    {
        ...
        {
            &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Unsupported Feature Combination NOR booting on platform $bb!", __FILE__, __LINE__);
        }
        ...
        if (defined $zimage and $zimage eq 'TRUE')
        {
            ...
            {
                &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Unsupported Feature Combination : ZIMAGE + MBA or FOTA!", __FILE__, __LINE__);
            }
        }
        if ($fota eq 'FOTA_DM')
        {
            ...
            {
                &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Unsupported Feature Combination : FOTA + ALICE!", __FILE__, __LINE__);
            }        	
        }        
...
sub config_query_flash_block_sz
{
    ...
    &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Query Block Size at $offset larger than available size!", __FILE__, __LINE__) if ($offset > $flash_size);

    my $href   = $norflash_info_blk_href;
    &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Block Size Information Unavailable, Please check EMI DataBase and custom_flash.c!", __FILE__, __LINE__) unless defined $href;
    ...
    &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Unreachable!", __FILE__, __LINE__);
}
...
sub config_query_next_block_aligned_addr
{
    ...
    &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Query Block-aligned addr at ($addr + $offset) larger than available size!", __FILE__, __LINE__) if (($addr+$offset) > $flash_size);
    ...
    &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Query Block-aligned addr at $tmp_addr larger than available size!", __FILE__, __LINE__) if ($tmp_addr > $flash_size);
    return $tmp_addr;
}
...
sub layout3_secusb_rom_ev
{
    ...
    my %BBtbl_RVCT =
       ...
       #map {print $_, "\n";} keys %BBtbl;
    &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Unsupported Feature Combination SECURE+USB-Download or SECURE+FOTA on $bb!", __FILE__, __LINE__) if not exists $BBtbl_RVCT{$bb}; 
    my $func;
    my $compiler = &$feature_query_function_ptr('compiler');
    if ($compiler eq 'RVCT')
    {
...
sub layout1_primaryrom_ev
{
    ...
    my %BBtbl =
       ...
       #map {print $_, "\n";} keys %BBtbl;
    &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Unsupported Feature Combination NFB on $bb!", __FILE__, __LINE__) if not exists $BBtbl{$bb}; 
    if ((&sysUtil::is_arm9($bb) != 1) and (&sysUtil::is_arm11($bb) != 1))
    {
        &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: NFB PRIMARY MAUI size $ram_size_value cannot < 8MB on ARM7 platforms!", __FILE__, __LINE__) if $ram_size_value < (8 * 1024 * 1024);
    }
    my $func = $BBtbl{$bb};
    return &$func;
}
...
sub layout0_rom_ev
{
    ...
    my %BBtbl_RVCT =
       ...
       #map {print $_, "\n";} keys %BBtbl;
    &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Unsupported Feature Combination Default on $bb!", __FILE__, __LINE__) if not exists $BBtbl_RVCT{$bb}; 
    my $func;
    my $compiler = &$feature_query_function_ptr('compiler');
    if ($compiler eq 'RVCT')
    {
...
__LARGEPOOLTEMPLATE
    my %BBtbl =
       ...
       #map {print $_, "\n";} keys %BBtbl;
    &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: unsupported $bb!", __FILE__, __LINE__) if not exists $BBtbl{$bb}; 
    # --- --- --- 
    return $BBtbl{$bb};
}
...
__SBNORFLASHTEMPLATE
    ...
    my %BBtbl =
       ...
       #map {print $_, "\n";} keys %BBtbl;
    &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: Unsupported Feature Combination ESB on $bb!", __FILE__, __LINE__) if not exists $BBtbl{$bb}; 
    # --- --- --- 
...
sub DSP_MT6256_DSPROM
{
	...
	{
		&sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: DSP_MT6256 can't exist with original DSP regions", __FILE__, __LINE__); 
	}
	my $template;
	if( $bb eq "MT6256")
	{
...
__TEMPLATE_6575
    ...
    &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: unsupported $bb!", __FILE__, __LINE__) if(!defined $strCODE_Base or !defined $strCODE_Size or !defined $strDATA_Base or !defined $strDATA_Size ); 
    ...
    if(&sysUtil::is_CR4($bb) == 1)
    {
        ...
        &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: unsupported $bb - intsram HW boundary!", __FILE__, __LINE__) if(!defined $strHW_Base or !defined $strHW_Size); 
...
sub DCMP_GetCombination
{
    ...
    if($nSize==0)
    {
		    &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: DCM Compression combination=0, please check feature option or turn off DCM Compression.", __FILE__, __LINE__);
    }
    return \@combination;
}
...
sub DCMP_code_layout
{
    ...
    {
        &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: DCM Compression should be 32B aligned!", __FILE__, __LINE__);
    }
...
{
    &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: the layout of extsram has not been generated", __FILE__, __LINE__) 
...
sub WarnMinus
{
    ...
    &sysUtil::sysgen_die("pcore/tools/scatGenLib.pl: The Size($nNumber) is not correct. $error_msg", $file, $line_no) if($nNumber < 0);
}
