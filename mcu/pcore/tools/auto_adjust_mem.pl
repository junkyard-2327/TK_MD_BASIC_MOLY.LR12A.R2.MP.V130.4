...
#* .
#*
#* 12 16 2015 ray.lin-wang
#* [MOLY00153288] [LR12][SystemService][Auto-Gen] Apply per-core small data section
#* .
#*
...
sub ckLinkError{
  ...
  if(-e $AAPMC_LINKLOG) {
    ...
    while (<FILE_HANDLE>) {
      $_ =~ s/\s+$//;
      if ($_ =~ /is not within region \`(RAM|ROM|VRAM|MCURO_HWRW|MCURW_HWRW|DSP_TXRX|IOCU|CORE0|CORE1)\'|will not fit in region \`ROM\'$/) {
        print "[Link Error] $_\n" if($DEBUG);
        $link_err_str .= $_."\n";
        $CK_LK_ERR = 1;
      } 
    ...
    {# enable this to generate dummy sym files for memory utility
      open (FILE_HANDLE, $AAPMC_LINKLOG) or &error_handler("cannot open $AAPMC_LINKLOG!", __FILE__, __LINE__);
      while (<FILE_HANDLE>) {
        $_ =~ s/\s+$//;
        if ($_ =~ /small-data section exceeds \d+KB; lower small-data size limi/) {
          print "[Link Error] $_\n" if($DEBUG);
          $link_err_str .= $_."\n";
          $CK_LK_ERR = 1;
        } 
