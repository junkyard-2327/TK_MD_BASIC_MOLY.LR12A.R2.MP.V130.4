...
sub ldsGen_verno
{
    return " a0.21_R2 + ldsFrame.pm ".&ldsFrame::ldsFrame_verno;
           # a0.21_R2,     2018/01/17,  Tero,   Adjusted MT6771 default memory map
           # a0.20_R2,     2018/01/15,  Tero,   Modem MiniDump implementation
           # a0.19_R2,     2017/12/06,  Carl,   Reduce DSP SPACE for special ZION
           # a0.18_UMOLY,  2017/09/27,  Tero,   MT6771 memory map updated
...
sub GenL1DSP_ZI_SIZE
{
    ...
    if(BuildInfo::exist(BuildOPT::CMPL, "__DSP_DEFAULT_ASIC_WO_LTEPOS__"))
    {
        my %BBtbl_small_DSP_size = 
        (   # format: '"chip"' -> 'size'
            'MT6739'  => 0x00D00000,        # 13 MB		     
        );	
...
sub Default_MemorySetting  # Called by CollectMemorySetting($Setting_href),  fill in MEMORY
{
    ...
    SetExistentValueByDefault($Setting_href, "IOCU2_BASE",0x2CC00000);
    ...
    SetExistentValueByDefault($Setting_href, "L2C_LOCK_BASE",0x9cf00000);
    ...
    SetExistentValueByDefault($Setting_href, "CORE1_BASE",0x0d400000);
...
sub MT6771_MemorySetting  # Called by CollectMemorySetting($Setting_href),  fill in MEMORY
{
    ...
    #MCURO_HWRW
    SetExistentValueByDefault($Setting_href, "MCURO_HWRW_BASE",$nUROMBase +$nROMSize);
    SetExistentValueByDefault($Setting_href, "MCURO_HWRW_LEN",0x00C00000);
    ...
    #MCURW_HWRW
    SetExistentValueByDefault($Setting_href, "MCURW_HWRW_BASE",hex($Setting_href->{RAM_BASE})+hex($Setting_href->{RAM_LEN}));
    SetExistentValueByDefault($Setting_href, "MCURW_HWRW_LEN",0x00F00000);
    SetExistentValueByDefault($Setting_href, "DSP_TXRX_BASE",hex($Setting_href->{MCURW_HWRW_BASE})+hex($Setting_href->{MCURW_HWRW_LEN}));
    SetExistentValueByDefault($Setting_href, "DSP_TXRX_LEN",0x40000);
    SetExistentValueByDefault($Setting_href, "IOCU2_BASE",((hex($Setting_href->{DSP_TXRX_BASE})+hex($Setting_href->{DSP_TXRX_LEN})+0xfffff)&0x0ff00000)|0x20000000);
