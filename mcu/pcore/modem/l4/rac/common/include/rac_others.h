...
/*****************************************************************************
* FUNCTION
*  rac_query_sbp_feature
* DESCRIPTION
*  This function is used to check whether the sbp shall be enable or not
*  We chech customization first, if thereis no any entry match then we check original sbp setting
*  For now, we only use this function for SBP_RAC_CHANGE_VDP_WHEN_IMS_ONOFF, 
*  if you need to do some change for this SBP, please remember to use this function
*
* PARAMETERS
*  sbp_md_feature_enum feature
*
* RETURNS
*  kal_bool    KAL_TRUE   sbp enable
               KAL_FALSE  sbp disable
* GLOBALS AFFECTED
*  we use this function to replace the sbp API: sbp_query_md_feature_by_ps which used at code  
*****************************************************************************/
kal_bool rac_query_sbp_feature(sbp_md_feature_enum feature);
