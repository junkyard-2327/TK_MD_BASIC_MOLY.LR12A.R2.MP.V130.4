...
extern void phb_check_pending_L4C_response(void);
...
/* under construction !*/
	...
	if (phb_util_is_cdma_card())
	{
		...
		{
	      phb_check_pending_L4C_response();  
		}
...
/*****************************************************************************
 * FUNCTION
 *  phb_check_pending_response
 * DESCRIPTION
 *  This is API to use when L4C waiting for any primitive response or not
 * PARAMETERS
 *  void
 * RETURNS
 *  True if succeed.
...
void phb_check_pending_L4C_response(void)
{
	kal_uint8 i;
	control_block_type *control_block = NULL;
    ...
    for (i = 0; i < PHB_CONCURRENCY_DEGREE; ++i)
	{
	  ...
	  {   
		  kal_trace(TRACE_INFO, INFO_PHB_L4C_WRONG_RES_IF_CB, i, control_block->ID, control_block->res_msg_id, control_block->cnf_msg_id);
		  kal_trace(TRACE_ERROR, ERROR_PHB_ABNORMAL);
...
void phb_primitive_mismatch_err_handler(control_block_type *control_block)
{
	...
	switch (control_block->cnf_msg_id)
	{
		...
		case MSG_ID_L4CPHB_APPROVE_REQ:
		{
		  ...
		  l4cphb_approve_cnf->type = PHB_NONE;
		  l4cphb_approve_cnf->src_id = control_block->src_id;
		  local_param_ptr = (local_para_struct*) l4cphb_approve_cnf;
		  l4cphb_approve_cnf->result = PHB_ERRNO_FAIL;
          l4cphb_approve_cnf->esc = 0;
		  l4cphb_approve_cnf->cause = SIM_CMD_FAIL;
		  phb_send_ilm(MOD_L4C, MSG_ID_L4CPHB_APPROVE_CNF, local_param_ptr, NULL);
          break;
		}
	}
}
