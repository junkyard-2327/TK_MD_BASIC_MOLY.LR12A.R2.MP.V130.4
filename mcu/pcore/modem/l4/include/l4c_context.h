...
typedef struct
{
    /* L4C owner can extend the pending item here */
//    l4c_source_id_enum  src_id;       // not used currently
    l4c_action_enum     pending_action;
...
/*#ifdef __HOMEZONE_SUPPORT__   
   ...
   */
   ...
   /* Guarding the card type switch duration */
   kal_bool    sim_card_type_switch_ongoing; //Only for ESIMAPP status floating during card type switch
...
//evshed_create, mtk02285, 20110328: remove base timer
    ...
    /* If AT command encountered cmd conflict situation, L4C could make use of this queue to suspend the action */
