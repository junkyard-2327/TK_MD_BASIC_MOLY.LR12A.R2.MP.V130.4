 ...
 * [MOLY00322937] ?信副卡不能注?VOLTE - notify C2K call end to L4 (VDM part)
 * .[R2] Send AT+EHVOLTE=0,2 to notify L4 C2K call end in order not to block switching to SRLTE mode
 *
 * 03 20 2018 yulong.chang
 * [MOLY00313620] [Zion][O1][BSP+]shows calling screen for always
 * 	
...
/******************************************************************************
 * AT+EMCSS=1,[<available_rat>,<prefer_rat>,<is_new_call>,<trans_id>]
...
/******************************************************************************
 * AT+EFUN=<fun>/AT+CFUN=<fun>
 * Set is_rf_on through this AT command
 *****************************************************************************/
typedef struct {
    LOCAL_PARA_HDR
    kal_bool            state;      
} vdm_atp_rf_state_ind_struct;


/******************************************************************************
 * +ESPLIND: <ind>,<rsrp_in_qdbm>
 *  rsrp_in_qdbm is not used by VDM
 * indication for signal power level
 *****************************************************************************/
typedef struct 
{
    LOCAL_PARA_HDR
    vdm_spl_ind_enum ind;
} vdm_atp_signal_power_level_ind_struct;
