...
BEGIN_TRACE_MAP(MOD_OSTD)
    ...
    TRC_MSG(OSTD_LEAVE_SLEEP_TRG4, "OSTD_SM : %Mslp_lock_type_e, OST_ISR = 0x%x, PWR_off_to_on = 0x%x, PWR_status = 0x%x, ForceOn = 0x%x, F32K_WkUp = 0x%08x 0x%08x, NonF32K_WkUp = 0x%08x 0x%08x, Slave_Req = 0x%08x, Dbg_Req = 0x%08x 0x%08x, recently wakeup timer = %s, CurFRC = %u")
