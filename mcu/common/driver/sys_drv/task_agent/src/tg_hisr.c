...
//L2Cache Locked...
//typedef struct{
TG_L2C_LOCK_ZI kal_hisrid tg1_hisr_id;
TG_L2C_LOCK_ZI kal_hisrid tg2_hisr_id;
...
//	{
		kal_hrt_activate_hisr(tg1_hisr_id);
//	}
//	else
//	{
//		kal_activate_hisr(tg1_hisr_id);
//	}
}
...
TG_L2C_LOCK_CODE void tg_group2_trigger(TG_TRIGGER_MODE mode)
{
	if(mode != TG_TRIGGER_GPT)
	{
		kal_hrt_activate_hisr(tg2_hisr_id);
	}
	else
	{
		kal_activate_hisr(tg2_hisr_id);
	}
...
{
	kal_activate_hisr(tg2_hisr_id); //activate by hmu task
}
#else  /* __LP_SCHEDULE_HMU_RESCHEDULE__ */
...
void tg_hisr_init(void)
{
    ...
    if (NULL == tg1_hisr_id)
    {
        tg1_hisr_id = kal_init_hisr(TG1_HISR);
    }
    if (NULL == tg2_hisr_id)
    {
        tg2_hisr_id = kal_init_hisr(TG2_HISR);
    }
#if defined(__LP_SCHEDULE_HMU_RESCHEDULE__)
    invoke_hmu = 1;
