...
//#define TK6291_NO_SPEECH_HW
  ...
  #define  MAX_HISR_HANDLER       13 // 3g, pnw, DACA for bt, vm, epl, bgsnd(ul+dl), record, cvsd
#endif //#if defined(__VOLTE_SUPPORT__)
#else // no 3g
#define  MAX_HISR_HANDLER       11 // pnw, DACA for bt, vm, epl, bgsnd(ul+dl), record, cvsd
...
void L1Audio_CCSetEvent( void )
{
   ...
   kal_hrt_activate_hisr( l1audio.hisr );
}
...
static void Audio_Wake_DSP(uint16 audio_id, kal_bool flag)
{
   ...
   if(flag)
   {
      ...
      }
      {
      	 uint32 I = 0 ;
      	 kal_uint32 core_id;
      	 kal_runtime_task_affine_to_current_cpu();
         ...
         kal_runtime_task_affinity_resume(kal_get_current_task());
...
void l1audio_console_handler(kal_char *string)
{
   ...
   else if(strcmp(string, "SPV_AWB_PATTERN")==0) 
   {
        ...
        SAL_TestMode_Config(7, SAL_DSP_TESTMODE_2G_FR);
   }
   else if(strcmp(string, "TestMode_2G_HR")==0) {
        SAL_TestMode_Config(7, SAL_DSP_TESTMODE_2G_HR);
   }
   else if(strcmp(string, "TestMode_2G_EFR")==0) {
        SAL_TestMode_Config(7, SAL_DSP_TESTMODE_2G_EFR);
   }
   else if(strcmp(string, "TestMode_2G_AMR")==0) {
        SAL_TestMode_Config(7, SAL_DSP_TESTMODE_2G_AMR);
   }
   else if(strcmp(string, "TestMode_2G_AWB")==0) {
        SAL_TestMode_Config(7, SAL_DSP_TESTMODE_2G_AWB);
   }
   else if(strcmp(string, "TestMode_3G_AMR")==0) {
        SAL_TestMode_Config(7, SAL_DSP_TESTMODE_3G_AMR);
   }
   else if(strcmp(string, "TestMode_3G_AWB")==0) {
        SAL_TestMode_Config(7, SAL_DSP_TESTMODE_3G_AWB);
   }
   else if(strcmp(string, "TestMode_4G_AMR")==0) {
        SAL_TestMode_Config(7, SAL_DSP_TESTMODE_4G_AMR);
   }
   else if(strcmp(string, "TestMode_4G_AWB")==0) {
        SAL_TestMode_Config(7, SAL_DSP_TESTMODE_4G_AWB);
   }
   else if(strcmp(string, "TestMode_C2K_EVA")==0) {
        SAL_TestMode_Config(7, SAL_DSP_TESTMODE_C2K_EVRCA);
   }
   else if(strcmp(string, "TestMode_C2K_EVB")==0) {
        SAL_TestMode_Config(7, SAL_DSP_TESTMODE_C2K_EVRCB);
   }
