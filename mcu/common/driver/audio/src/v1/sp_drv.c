...
{
	return ((l1sp.devInfo == SPH_DEVINFO_BT_CVSD_MSBC) || (l1sp.devInfo == SPH_DEVINFO_BT_PCM));
}
...
void SP_MuteController() // central controll, and placed between L1Audio_SetFlag and L1Audio_ClearFlag
{
   ...
   kal_trace(TRACE_INFO, L1SP_MUTE_CONTROL_STATUS, 2, l1sp.isULMute, l1sp.isDLMute, l1sp.codec_mute);
		...
		AM_Mute_UL_POS_EN_Speech(0 != l1sp.isULMute);
}
...
void L1SP_Speech_On( uint8 RAT_Mode )
{
   ...
   {
#if defined(__CVSD_CODEC_SUPPORT__) 
      if((l1sp.devInfo == SPH_DEVINFO_BT_CVSD_MSBC) && !BT_SCO_IS_SPEECH_ON())
      {
         kal_bool is_WB = KAL_FALSE;
...
#if defined(__CVSD_CODEC_SUPPORT__) 
   if(l1sp.devInfo == SPH_DEVINFO_BT_CVSD_MSBC)
   {
      kal_bool is_WB = KAL_FALSE;
      kal_uint8 voice8kMode = SP_GetDevSamplingRate();
...
//FIXME: for phone call usage
   ...
   l1sp_send_codec_status_notify(0xff);
...
void SP_SetDevicePath(sp_device_info_t newDev, kal_uint8 voice8kMode)
{
	...
	if(SPH_DEVINFO_NORMAL == beforeDev && SPH_DEVINFO_BT_PCM== newDev) {
		// off
	...
	else if (SPH_DEVINFO_BT_PCM == beforeDev && SPH_DEVINFO_NORMAL == newDev) {		
		// off 
		if(true == isAmInSpeechState) {
			if(true == isSpeechOn) {
				AM_SpeechOff(1); // turn off 8k and speech off
	...
	else if (SPH_DEVINFO_BT_PCM == beforeDev && SPH_DEVINFO_BT_PCM == newDev) {	// change sampling rate only		
		// off 
		if(true == isAmInSpeechState) {
			if(true == isSpeechOn) {
				AM_SpeechOff(1); // turn off 8k and speech off
...
#if defined(__CVSD_CODEC_SUPPORT__) 	
	else if(SPH_DEVINFO_NORMAL == beforeDev && SPH_DEVINFO_BT_CVSD_MSBC == newDev) {
		// no off
	...
	else if(SPH_DEVINFO_BT_CVSD_MSBC == beforeDev && SPH_DEVINFO_BT_CVSD_MSBC == newDev) {
		if (isSpeechOn && BT_SCO_IS_SPEECH_ON())
	    {
			BT_SCO_SPEECH_OFF();
		}
