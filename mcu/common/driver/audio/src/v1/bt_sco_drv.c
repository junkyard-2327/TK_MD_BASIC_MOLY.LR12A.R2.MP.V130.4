...
static void btsco_hisr_internal()
{
   ...
   // consys return 0xDEADFEED (DE(Congliang Liao))
   //1. connsys in sleep
   ...
   kal_prompt_trace(MOD_L1SP, "[SCO] btsco_hisr TX disable isr period %d", ust_us_duration(curr_frc, latest_us));
   curr_frc = ust_get_current_time();
   if(btsco.pTX)
   {
   ...
   latest_us = ust_get_current_time();
   kal_prompt_trace(MOD_L1SP, "[SCO] btsco_hisr RX disable isr period %d", ust_us_duration(curr_frc, latest_us));
}
...
static void btsco_hisr(void *pData)
{
   ...
   *BT_SCO_HW_REG_CONTROL &= ~BT_CVSD_CLEAR;
   ...
   //kal_give_spinlock(bt_lockId);
   latest_us = ust_get_current_time();
   kal_prompt_trace(MOD_L1SP, "[SCO] btsco_hisr disable isr period %d", ust_us_duration(curr_frc, latest_us));
...
static void btsco_process_RX_MSBC()
{
   ...
   while(1)
   {
      ...
      {
      	 kal_prompt_trace(MOD_L1SP, "mike msbc data ok");
      ...
      {//wrong header, but packet not lost, sync to next 30 byte block (should only happenes on packetsize 30)
         ...
         if(status == -1)
         {
            kal_prompt_trace(MOD_L1SP, "mike not found");
         }
      }
	    else
	    {//packet lost at first frame, fail to decode
	    	kal_prompt_trace(MOD_L1SP, "mike msbc data and header fail");
		    status = -1;
		    btsco.pRX->iPacket_r++;
      }
