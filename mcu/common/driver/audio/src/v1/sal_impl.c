...
volatile uint16* SALI_Parameter_GetAddr(uint8 type)
{
	...
	switch(type)
	{
		...
		case SALI_PARAMETER_TYPE_MODE_NB:
			addr = SphEXTCUIF->tSchEMP.ModPar_NB;
			break;
		case SALI_PARAMETER_TYPE_MODE2_NB:
			addr = &(SphEXTCUIF->tSchEMP.ModPar_NB[16]);
			break;
		case SALI_PARAMETER_TYPE_MODE3_NB:
		    addr = &(SphEXTCUIF->tSchEMP.ModPar_NB[32]);
			break;
		case SALI_PARAMETER_TYPE_MODE_WB:
			addr = SphEXTCUIF->tSchEMP.ModPar_WB;
			break;
		case SALI_PARAMETER_TYPE_MODE2_WB:
			addr = &(SphEXTCUIF->tSchEMP.ModPar_WB[16]);
			break;
		case SALI_PARAMETER_TYPE_MODE3_WB:
			addr = &(SphEXTCUIF->tSchEMP.ModPar_WB[32]);
			break;
		case SALI_PARAMETER_TYPE_MODE_SWB:
		    addr = SphEXTCUIF->tSchEMP.ModPar_SWB;
		    break;
...
void SALI_EPL_GetFrame(Sal_EPL_Frame *eplfrm)
{
	...
	eplfrm->ul4_pos_buf = (uint16*)SphEXTCUIF->tSchDPL.DPLBuf_UL_4;
...
#if (defined(__SAL_SPH_COD_MAX_BW_SWB__) || defined(__SAL_SPH_COD_MAX_BW_FB__))
    SphCUIF->tCodEVS.PCMBW = SphCUIF->tCodEVS.tEncPar.NWReqBW;
#else
    SphCUIF->tCodEVS.PCMBW = SPH_SCH_BAND_WB;
#endif
...
#else
    SphCUIF->tCodEVS.tDecPar.PCMBW = SPH_SCH_BAND_WB;
#endif
