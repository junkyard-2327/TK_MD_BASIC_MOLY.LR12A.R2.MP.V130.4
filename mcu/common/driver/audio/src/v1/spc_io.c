...
kal_bool spcIO_sendMsgViaCCCI(kal_uint32 msg, kal_uint32 revMsg)
{
	...
	if(true == gSpcIo.isFactoryBoot) {
		kal_trace(TRACE_INFO, SPCIO_FACTORY_DATA_NO_SEND, msg);
		return true;
	}
...
kal_bool SpcIO_unknownMsgAckToAp(uint16 apMsg, uint32 data1, uint32 reserve)
{
	uint32 msgContent = SPCIO_CCCI_MSG_CONSTRCUT_CMD((apMsg|0x8000), data1);
...
void SpcIO_shmInit(void) // run under audio task
{
	...
	kal_trace(TRACE_INFO, SPCIO_GET_FACTORYINFO, feature_support.support_mask, boot_info.BootingStartID);	
	EXT_ASSERT(feature_support.support_mask == CCCI_RUNTIME_FEATURE_MUST_SUPPORT, feature_support.support_mask, 0, 0);
	gSpcIo.isFactoryBoot = (FACTORY_BOOT_ID == (boot_info.BootingStartID & 0x000000FF));
