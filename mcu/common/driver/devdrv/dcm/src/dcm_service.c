...
static void dcm_service_unmask_vpe_irq(kal_uint32 core_id, kal_uint32 vpe_id, kal_bool Check_hrt_Qbit)
{
    ...
    {/* Check 40Qbit due to we also disable VPE1's IRQ */
        ...
        if( dcm_service_hrt_DI_dur[core_id] > IRQ_DISABLE_MAX_DURATION_HRT && !L1_CHECK_ASSERT_BYPASS() )
        {
            #if 0
/* under construction !*/
/* under construction !*/
...
/* under construction !*/
            #endif
            IFDEF_PRODUCTION(EXT_ASSERT(0, dcm_service_hrt_DI_dur[core_id], dcm_service_hrt_DI_start[core_id], dcm_service_hrt_DI_end[core_id]));
        }
