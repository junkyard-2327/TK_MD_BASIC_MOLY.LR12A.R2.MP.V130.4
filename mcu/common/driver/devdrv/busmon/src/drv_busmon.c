 ...
 * 	[R2]Rollback busmon debug patch for MTBF issue
 *
 ...
 * 	
 * 	[Merlot][R2]add busmon debug patch for MTBF issue
 *
 * 03 23 2018 liang.yan
 * [MOLY00315775] BUS monitor build error with MT6761 option call for check in
 *
 * 03 23 2018 liang.yan
 * [MOLY00315775] BUS monitor build error with MT6761 option call for check in
 *
 * 01 09 2018 liang.yan
 * [MOLY00300824] [Cervino bringup merge back] Add build option MT6765 for bus monitor
 *
 * 09 21 2017 liang.yan
 * [MOLY00279174] [6293] HARQ BUS configuration development
 * 	
 * 	[R2]Busmon rollback to CL4430005
 *
...
#else  /*default config*/
       busmon_snap_mo_init();
       /*revert mdmcu busmon configuration for mpu violation*/
       //busmon_mpu_vio_debug();
       /*register callback*/
...
/* under construction !*/
#endif
#if 0  /*revert mdinfra busmon configuration for cirq hw issue*/
/* under construction !*/
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
void mdinfra_busmon_snap_init(void)
{
	...
	/* IP0: Read Address Trigger */
	trg_cfg.ip[BUSMON_IP0].rwsel = AXIMON_RWSEL_WRITE;
	trg_cfg.ip[BUSMON_IP0].master_id = 0;
	trg_cfg.ip[BUSMON_IP0].master_id_mask = AXIID_IA_ID_MASK_ALL;
	trg_cfg.ip[BUSMON_IP0].vpe_id = MON_VPE_0;
	trg_cfg.ip[BUSMON_IP0].vpe_id_mask = AXI_VPE_ID_MASK_ALL;  
	...
	trg_cfg.ip[BUSMON_IP0].addr = 0xA8240F50;              /*0xA0070C00 ~ 0xA0070CFF, 0xB0070C00 ~ 0xB0070CFF*/
	trg_cfg.ip[BUSMON_IP0].addr_mask = AXIMON_MATCH_ALL;
	trg_cfg.ip[BUSMON_IP0].data_enable = KAL_FALSE; 		
	trg_cfg.ip[BUSMON_IP0].data = 0;
	trg_cfg.ip[BUSMON_IP0].data_mask = AXIMON_MATCH_NONE;
	...
	trg_cfg.ip[BUSMON_IP1].addr = 0xB8240F50;
	trg_cfg.ip[BUSMON_IP1].addr_mask = AXIMON_MATCH_ALL;         /*0xA0070C00 ~ 0xA0070CFF, 0xB0070C00 ~ 0xB0070CFF*/		 
	trg_cfg.ip[BUSMON_IP1].data_enable = KAL_FALSE; 	   
	trg_cfg.ip[BUSMON_IP1].data = 0;
	trg_cfg.ip[BUSMON_IP1].data_mask = AXIMON_MATCH_NONE; 
