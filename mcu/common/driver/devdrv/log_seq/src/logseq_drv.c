...
extern void ulsp_log_pb_dbg_info(void);
...
void logseq_drv_LS_irq_init(kal_uint8 ch_id)
{
    if(ch_id == LS_MCU_CH){
        ...
        LSInternalInfo[ch_id].logseq_hisrid= kal_init_hisr(LS_MCU_HISR);
        ...
        LSInternalInfo[ch_id].logseq_hisrid = kal_init_hisr(LS_DSP_HISR);
...
void logeq_drv_add_abort_timeout_info(kal_uint8 ch_id)
{
	//kal_uint32 i, val;
...
kal_uint32 logseq_drv_cal_emi_ptr_len_with_thrs_buf_wrapping(kal_uint8 ch_id, LOG_SEQ_Thrs_Buf_Info* thrs_buf,kal_uint32 base_addr,kal_uint32 buf_size){
	...
	if(0 == emi_wrap_bit){ 
		...
		//if the wrap bit is set: ((WHQA00032633) & (WHQA00035111))
...
{	
    ...
    {
    	//if no 128-byte data come, the write pointer = base addr and write aligned ptr = 0
    	if(write_aligned_ptr == 0){ write_aligned_ptr = base_addr;}
    	//the solutions to oppo issues for continous LOGSEQ_DSP_HISR interrupt to cause other HRT task failed
...
#if defined(__MTK_INTERNAL__)
static void logseq_drv_ls_hang_dbg_info(kal_uint8 ch_id){
					...
					if((switch_swwa_cnter&0xFFFF0000) == 0x3A1B0000){
						if(ch_id == LS_MCU_CH){
							...
							if(mcu_cntr < SWWA_THRESHOLD){ //change
								...
								//	TODO: debug information in MTK logger flavor
								ulsp_log_pb_dbg_info();
								logseq_drv_ls_hang_dbg_info(ch_id);
								LS_PRINT_ASSERT("[LS] ch_id %d the LS mcu swwa overflow\r\n",ch_id);
#endif
							}
						}else if(ch_id == LS_DSP_CH){
							dsp_cntr = (switch_swwa_cnter&0xFF);
							if(dsp_cntr < 255){ //change
								dsp_cntr+=1;
...
{	
	...
	if(write_align_ptr == 0){
		...
		if(logseq_drv_cal_idx_increase(emi_read_ptr, len, buf_size) != (write_align_ptr-base_addr)){
		    ...
		    //dbg_print("remain(%x), write_align_addr(%x), write_align_idx(%x), emi_read_ptr(%x), len(%x)\r\n",remain_sz,write_align_ptr,write_align_ptr-base_addr,emi_read_ptr,len);
...
kal_uint32 logseq_drv_stop_im(kal_uint8 ch_id)
{
	...
	if(LSInternalInfo[ch_id].logseq_backup_sta == LS_RESTORE_SUCCESS){
		...
		if(rval == LS_ABORT_TIMEOUT)
		{
			...
			//LS_PRINT_ASSERT("[LS] ChID:%d abort timeout", ch_id);
			//LS abort timeout sw workaround
			logseq_drv_abort_timeout_recover(ch_id);
		}else{
			logseq_drv_handle_emi_buf(ch_id);
			// LS backup register info
...
{	
	LOG_SEQ_Internal_Info *interInfo = &LSInternalInfo[ch_id];
    // restore EMI write pointer & EMI buffer remain size
    ...
    // restore EMI wrapped state WHQA00032633(Gen93)
...
kal_uint32 logseq_drv_force_rls_ths_count(kal_uint8 ch_id)
{
	...
	LOG_SEQ_Internal_Info *interInfo = &LSInternalInfo[ch_id];  
	if((is_drv_init & (1<<ch_id))){
	  if(KAL_TRUE==logseq_drv_api_try_lock(ch_id, LS_FORCE_RLS_LOCK)){
		logseq_drv_mask_irq(ch_id);//take atomic spin lock
    ...
    if(ch_id == LS_MCU_CH){
        while((DRV_Reg32(MCU_LS_L2_GLB_FLUSH_STATUS)&0x1) != 0x1){
            if(logseq_drv_get_us_diff(s_tick) > WHOLE_SYS_TIMEOUT){
                ...
                DRV_Reg32(LS_REG(ch_id,DSP_LS_L2_WRITE_ADDR)),DRV_Reg32(LS_REG(ch_id,DSP_LS_L2_READ_ADDR)));
...
void logseq_lisr_mcu(kal_uint32 vector)
{
    ...
    if(KAL_TRUE == logseq_drv_api_try_lock(LS_MCU_CH, LS_IRQ_LOCK)){
        ...
        kal_activate_hisr(LSInternalInfo[LS_MCU_CH].logseq_hisrid);
...
void logseq_lisr_dsp(kal_uint32 vector)
{
    ...
    if(KAL_TRUE == logseq_drv_api_try_lock(LS_DSP_CH, LS_IRQ_LOCK)){
        ...
        kal_activate_hisr(LSInternalInfo[LS_DSP_CH].logseq_hisrid);
