...
static kal_bool SIM_ResetNoATR(kal_uint8 pow, sim_HW_cb *hw_cb)	//For normal case reset
{
    ...
    while (retry++ < 3)
    {
        ...
        else
        {
            ...
            if (usim_dcb->present == KAL_FALSE || SimCard->result == SIM_NOREADY)
            {
            ...
            kal_sprintf(hw_cb->dbgStr, "[%s] SimCard->result : %d\n\r", __func__, SimCard->result);
...
static kal_bool SIM_PTSProcess(kal_uint8 *TxBuffaddr, kal_uint8 Txlength, sim_HW_cb *hw_cb)	//Bool lalasun
{
            ...
            || (SimCard->result == SIM_INIPTSERR))
    {
        ...
        kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d][ERR]SIM_INIPTSERR\n\r", hw_cb->simInterface);
        DRV_ICC_print_str(hw_cb->dbgStr);
        return KAL_FALSE;
    }
        ...
        while (KAL_TRUE)
        {
            ...
            if (SIM_FIFO_GetLev() == 0)
            {
                if (index > PPS_LEN)
                {
                    ...
                    kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d][ERR] Too many PTS:%d\r\n", hw_cb->simInterface, index);
                    DRV_ICC_print_str(hw_cb->dbgStr);
                }
...
void SIM_HISR_MT6306_Common(sim_HW_cb *hw_cb, kal_uint32 sim_int)
{
    ...
    kal_sprintf(hw_cb->hisrDbgStr, "[SIM_DRV:%d]SIM HISR int: %x, IRQEN:%x, CONF:%x\r\n", hw_cb->simInterface, sim_int, DRV_Reg32(SIM0_BASE_ADDR_MTK + SIM_IRQEN_MTK), DRV_Reg32(SIM0_BASE_ADDR_MTK + SIM_CONF_MTK));
    ...
    {
        kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d][ERR]SIM_STS_TXERR\r\n", hw_cb->simInterface);
        DRV_ICC_print_str(hw_cb->dbgStr);
        if (SimCard->State == SIM_PROCESSCMD)
        {
    ...
    {
        kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d][ERR]SIM_STS_RXERR\r\n", hw_cb->simInterface);
        DRV_ICC_print_str(hw_cb->dbgStr);
        SimCard->recDataErr = KAL_TRUE;
    ...
    {
        kal_sprintf(hw_cb->hisrDbgStr, "[SIM_DRV:%d][ERR]SIM_STS_NATR\r\n", hw_cb->simInterface);
        DRV_ICC_print_str(hw_cb->hisrDbgStr);
        SIM_DisAllIntr();
...
void sim_nullByteIssueGptTimeoutMT6306(void *parameter)
{
    ...
    {
        kal_sprintf(hw_cb->dbgStr, "sim_nullByteIssueGptTimeout_1:cmd finished before GPT timeout");
        DRV_ICC_print_str(hw_cb->dbgStr);
    }
    else /*the GPT timer is used to find out these cases, it means we still haven't complete the CMD for so long duration*/
    {
        ...
        if (0x60 == SW1)
        {
            ...
            if (38 < SimCard->sim_nullByteIssueNullCount)
            {
                ...
                SIM_DisAllIntr();
                kal_sprintf(hw_cb->dbgStr, "[SIM:%d] : null timeout %d\n\r", __LINE__, SimCard->sim_nullByteIssueNullCount);
                DRV_ICC_print_str(hw_cb->dbgStr);
        ...
        else  /*received is not null*/
        {
            ...
            if (48 < SimCard->sim_nullByteIssuenonNullCount)
            {
                ...
                SIM_DisAllIntr();
                kal_sprintf(hw_cb->dbgStr, "[SIM:%d] : non-null timeout %d\n\r", __LINE__, SimCard->sim_nullByteIssuenonNullCount);
                DRV_ICC_print_str(hw_cb->dbgStr);
                SimCard->timeout = KAL_TRUE;
                SIM_SetEvent_MT6306(SimCard, SIM_NULLTIMEOUT, hw_cb);
            }
...
/* under construction !*/
    ...
    {
        kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d][ERR]Illegal SW:%x\r\n", hw_cb->simInterface, SW);
        DRV_ICC_print_str(hw_cb->dbgStr);
        *Error = KAL_TRUE;
    }
    ...
    for (;;)
    {
                 ...
                 (sw1 == SW1_WARN1 || sw1 == SW1_WARN2 || ((status != SIM_SW_STATUS_OK) && ((sw1 & 0xf0) == 0x90) && (sw1 != 0x91) && (sw1 != 0x92))))
        {
            ...
            *isSW6263 = KAL_TRUE; //set 0x62xx 0x63xx flag
            ...
            {
                kal_sprintf(hw_cb->dbgStr, "[ERR]CLA:%x, INS:%x, isSW6263:%d, warn:%d, rx_len:%d, *rxSize:%x", *txData, *(txData + 1), sw1, sw2, rx_len, *rxSize);
                DRV_ICC_print_str(hw_cb->dbgStr);
