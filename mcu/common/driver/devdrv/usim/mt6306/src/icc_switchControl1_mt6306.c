...
//                only used in test mode for T1 testing.
    ...
    {
        kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d][ERR]PPS exchange fail\n\r", hw_cb->simInterface);
...
//			DRV_ICC_print(hw_cb, SIM_PRINT_L1SIM_CMD_TRC20, hw_cb->mtk_baseAddr, SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_ATIME_MTK), SIM_Reg32(0x80000450), hw_cb->mtk_pdnAddr, 0x1116);
            ...
            DRV_ICC_print(hw_cb, SIM_PRINT_L1SIM_CMD_TRC20, SIM_Reg(SIM0_BASE_ADDR_MTK + 0x00), SIM_Reg(SIM0_BASE_ADDR_MTK + 0x4), SIM_Reg(SIM0_BASE_ADDR_MTK + 0x8), SIM_Reg(SIM0_BASE_ADDR_MTK + 0x10), SIM_Reg(SIM0_BASE_ADDR_MTK + 0x20));
            DRV_ICC_print(hw_cb, SIM_PRINT_L1SIM_CMD_TRC20, SIM_Reg(SIM0_BASE_ADDR_MTK + 0x24), SIM_Reg(SIM0_BASE_ADDR_MTK + 0x34), SIM_Reg(SIM0_BASE_ADDR_MTK + 0x60), SIM_Reg(SIM0_BASE_ADDR_MTK + 0x68), usim_dcb->ev_status);
            DRV_ICC_print(hw_cb, SIM_PRINT_L1SIM_CMD_TRC20, SIM_Reg(SIM0_BASE_ADDR_MTK + 0x6c), SIM_Reg(SIM0_BASE_ADDR_MTK + 0x70), SIM_Reg(SIM0_BASE_ADDR_MTK + 0x74), SIM_Reg32(SIM0_BASE_ADDR_MTK + 0x90), drv_get_current_time());
...
static void usim_timeout_handler(sim_HW_cb *hw_cb)
{
    ...
    switch (usim_dcb->main_state)
    {
            ...
            // read the remaining bytes of ATR
        {
            ...
            USIM_SET_EVENT_Multiple(usim_dcb);
            kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d][ERR] ATR TOUT, usim_dcb->ev_status Status:%x\n\r", hw_cb->simInterface, usim_dcb->ev_status);
            DRV_ICC_print_str(hw_cb->dbgStr);
...
static void usim_hisr_common(sim_HW_cb *hw_cb, kal_uint32 int_status)
{
    ...
    //DRV_ICC_print_str(hw_cb->dbgStr);
    kal_sprintf(hw_cb->hisrDbgStr, "[SIM_DRV:%d]USIM HISR int: %x, IRQEN:%x, CONF:%x\r\n", hw_cb->simInterface, int_status, DRV_Reg32(SIM0_BASE_ADDR_MTK + SIM_IRQEN_MTK), DRV_Reg32(SIM0_BASE_ADDR_MTK + SIM_CONF_MTK));
    ...
    if (int_status & SIM_STS_RXERR)
    {
        usim_set_timeout(0, hw_cb);
        usim_dcb->ev_status = USIM_RX_INVALID;
        kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d][ERR]RXERR,usim_dcb->ev_status:%x, int_status:%x\n\r", hw_cb->simInterface, usim_dcb->ev_status, int_status);
        DRV_ICC_print_str(hw_cb->dbgStr);
    ...
    if (int_status & SIM_STS_TXERR || int_status & SIM_STS_OV)
    {
        ...
        usim_set_timeout(0, hw_cb);
        usim_dcb->ev_status = USIM_RX_INVALID;
        kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d][ERR]TXERR,usim_dcb->ev_status:%x, int_status:%x\n\r", hw_cb->simInterface, usim_dcb->ev_status, int_status);
        DRV_ICC_print_str(hw_cb->dbgStr);
    }
    if (int_status & SIM_STS_NATR)
    {
        usim_dcb->ev_status = USIM_NO_ATR;
        kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d][ERR]No ATR\n\r", hw_cb->simInterface);
        DRV_ICC_print_str(hw_cb->dbgStr);
        USIM_SET_EVENT_Multiple(usim_dcb);
    }
    ...
    switch (usim_dcb->main_state)
    {
            ...
            /////dbg_print("ACTIVATION_STATE ");
        {
            ...
            else
            {
                ...
                kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d][ERR]Check HW Waveform for HW issue. TS:%x T0:%x usim_dcb->ev_status:%x, int_status:%x\n\r", hw_cb->simInterface, TS, T0, usim_dcb->ev_status, int_status);
                DRV_ICC_print_str(hw_cb->dbgStr);
            }
            USIM_SET_EVENT_Multiple(usim_dcb);
        }
