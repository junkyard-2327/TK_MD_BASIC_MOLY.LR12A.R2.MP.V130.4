...
kal_bool nvram_custom_config_sbp(sbp_reconfig_custom_param_struct *param_ptr)
{
        ...
        (NVRAM_DEFAULT_VALUE_POINT == nvram_get_default_value(NVRAM_EF_SBP_MODEM_CONFIG_LID, 1, (kal_uint8**)&default_sbp_feature)))
    {
        ...
        /* retrieve the current SBP NV if SBP_ATTR_NOT_RESET_NVRAM_WHEN_DYNAMIC_SBP */
        if (KAL_TRUE == param_ptr->is_in_dynamic_sbp)
        {
            sbp_md_feature_enum i = 0;
            kal_bool is_turned_on = KAL_FALSE;
            ...
            {
                if ((sbp_feature_attr_table[i] & SBP_ATTR_NOT_RESET_NVRAM_WHEN_DYNAMIC_SBP_BIT) == SBP_ATTR_NOT_RESET_NVRAM_WHEN_DYNAMIC_SBP_BIT)
                {
                    is_turned_on = sbp_query_md_feature_by_nvram(i, &sbp_feature_buf);

                    sbp_set_md_feature(i ,is_turned_on, default_sbp_feature);
                }
            }
        }            
        ...
        (NVRAM_DEFAULT_VALUE_POINT == nvram_get_default_value(NVRAM_EF_SBP_MODEM_DATA_CONFIG_LID, 1, (kal_uint8**)&default_sbp_data_buf)))
    {
        ...
        if (KAL_TRUE == param_ptr->is_in_dynamic_sbp)
        {
            ...
            {
                if ((sbp_data_attr_table[i] & SBP_ATTR_NOT_RESET_NVRAM_WHEN_DYNAMIC_SBP_BIT) == SBP_ATTR_NOT_RESET_NVRAM_WHEN_DYNAMIC_SBP_BIT)
                {
                    data = sbp_query_md_feature_data_by_nvram(i, &sbp_data_buf);

                    sbp_set_md_feature_data(i, data, default_sbp_data_buf);
                }
            }
        }
    ...
    else if (sbp_id == 1) //for CMCC
    {
        ...
        sbp_set_md_feature(SBP_OP01_SIM_LOCK, KAL_FALSE, (nvram_ef_sbp_modem_config_struct*)&sbp_feature_buf);
    ...
    else if (sbp_id == 6) //for Vodafone
    {
        ...
        sbp_set_md_feature(SBP_SUPPORT_GEA1, KAL_FALSE, (nvram_ef_sbp_modem_config_struct*)&sbp_feature_buf);
    #ifdef __A54_ALGORITHM_SUPPORT__
        sbp_set_md_feature(SBP_SUPPORT_GEA4, KAL_TRUE, (nvram_ef_sbp_modem_config_struct*)&sbp_feature_buf);
    #endif
    }
    ...
    else if (sbp_id == 12) //for VzW
    {
        ...
        sbp_set_md_feature(SBP_STATUS_POWERUP_1X_SCAN, KAL_FALSE, (nvram_ef_sbp_modem_config_struct*)&sbp_feature_buf);
        sbp_set_md_feature(SBP_STATUS_MIP, KAL_TRUE, (nvram_ef_sbp_modem_config_struct*)&sbp_feature_buf);
    ...
    else if (sbp_id == 17) //for DOCOMO
    {
        ...
        sbp_set_md_feature(SBP_UL1D_CS_CS3_THD_DOCOMO_EN, KAL_TRUE, (nvram_ef_sbp_modem_config_struct*)&sbp_feature_buf);
    }
    ...
    else if (sbp_id == 19) //for Telstra
    {
		  ...
		  sbp_set_md_feature(SBP_OP19_CLEAR_CODE_33, KAL_TRUE, (nvram_ef_sbp_modem_config_struct*)&sbp_feature_buf);
