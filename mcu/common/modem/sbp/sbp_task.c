...
extern reconfigure_module_info_struct *sbp_get_sbp_reconfigure_mod_tbl();
...
void sbp_dsbp_reconfigure_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    reconfigure_module_info_struct *module_tbl;
    ...
    for (i=0; i<total_mod; i++)
    {
        ...
        if ((module_tbl[i].reconfig_type & RECONFIG_DSBP_CHANGE) == 0)
        {
            continue;
        }

        /* Check if Gemini module and right protocol id */
        if (((module_tbl[i].gemini_check & CFG_GEMINI_MOD) == 0) &&
            (ps_id != PROTOCOL_1))
        {
            continue;
        }
