...
void imc_set_config(kal_uint32 op_id, sim_interface_enum sim_slot_id, kal_bool with_imsi_info, kal_uint8 imsi_mnc_len, kal_uint8 *mccmnc);
 ...
 *****************************************************************************/
 kal_bool nvram_custom_config_ims_profile(sbp_reconfig_custom_param_struct *param_ptr)
 {
    ...
    if(param_ptr->is_in_dynamic_sbp){
        ...
        imc_set_config(param_ptr->sbp_id, sim_slot_id, param_ptr->is_in_dynamic_sbp, imsi_mnc_len, &mccmnc[0]);
    }
    return KAL_FALSE;
 }
 #endif /* __MOD_IMC__ */
