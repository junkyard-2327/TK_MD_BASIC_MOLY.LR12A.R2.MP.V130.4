...
#define MAX_SUPPORT_SIM_TOTAL 2

#define UTK_TIMER_MANAGER_NUM 8
#define CUIM_BUFSIZE         256
...
#define C2K_UIM_INVALID_SAP INVALID_SAP 
...
#define UIM_UTK_MSG_CLASS   0x00A0
#define UIM_GSM_MSG_CLASS   0X00C0
#define UIM_NAM_MSG_CLASS   0x0100
#define UIM_APP_MSG_CLASS   0x0200
#define UIM_MNG_MSG_CLASS   0x0300
#define C2K_UIM_MSG_CLASS   0x0400
...
typedef   struct 
{
    ...
    kal_uint8        Data[CUIM_BUFSIZE]; /* Response data */
}   UimTermRspMsgT;
...
typedef   struct 
{
    ...
    kal_uint8        Data[CUIM_BUFSIZE];/* Envelope data */
}   UimEnvelopeMsgT;
/***********************************************
...
typedef   union
{
  ...
  {
     kal_uint8 blockID; 
     kal_uint8 reqIndex[2];
     kal_uint8 reqMaxEntry;
  } blockID1;
  struct
  {
     kal_uint8 blockID;
     kal_uint8 UZ_ID[2];
     kal_uint8 UZ_SID[2];
     kal_uint8 reqOffset[2];
     kal_uint8 reqMaxSize;        
  } blockID2;
  struct
  {
     kal_uint8 blockID;
...
}   UimPuzlConfigurationBlockParamT;

typedef   struct 
{
    UimRspInfoT  UimRspInfo;
    UimPuzlConfigurationBlockParamT UimPuzlConfigBlockParam;
}   UimPuzlConfigurationRequestMsgT;
...
typedef   struct 
{
    UimRspInfoT  UimRspInfo;
    kal_uint8        Block;
}   UimMmdConfigurationRequestMsgT;
...
typedef   struct 
{
    ...
    kal_uint8        Len;
    kal_uint8        Data[UIM_MAX_OTA_DATA_BURST_SIZE];
}   UimMmdDownloadRequestMsgT;

typedef   struct 
{
    UimRspInfoT  UimRspInfo;
    kal_uint8        Block;
}   UimMmsConfigurationRequestMsgT;
...
typedef   struct 
{
    ...
    kal_uint16       ReqOffset;
    kal_uint8        ReqMax;
}   UimSystemTagConfigurationRequestMsgT;
...
typedef   struct 
{
    ...
    kal_uint8        Len;
    kal_uint8        Data[UIM_MAX_OTA_DATA_BURST_SIZE];
}   UimSystemTagDownloadRequestMsgT;
...
typedef   struct 
{
    ...
    kal_uint16       ReqOffset;
    kal_uint8        ReqMax;
}   UimMmssConfigurationRequestMsgT;
...
typedef   struct 
{
    ...
    kal_uint8        Len;
    kal_uint8        Data[UIM_MAX_OTA_DATA_BURST_SIZE];
}   UimMmssDownloadRequestMsgT;

typedef   struct 
{
    UimRspInfoT  UimRspInfo;
    kal_uint8        keyInUse;
    kal_uint8        startStop;   /* 0x00 for start, 0x01 for stop */
    kal_uint8        randSM[8];
}   UimSecureModeMsgT;
...
{
    UimRspInfoT  UimRspInfo;
	kal_uint8        putGet;     /* 0x00 for put, 0x01 for get     */
	kal_uint16       cryptoSync; /* High octet for first Byte of cryptoSync */
}   UimFreshMsgT;

typedef   struct 
{
    UimRspInfoT  UimRspInfo;
	kal_uint16       keyID;      /* High octet for first Byte of KEY_ID */
}   UimServiceKeyGenerationRequestMsgT;

typedef   struct 
{
   struct
   {
     kal_uint8     BcmcsFlowIDLen;
     kal_uint8*    BcmcsFlowIDPtr;
     kal_uint8     BakIDLen;
     kal_uint8*    BakIDPtr;
     kal_uint8     SKRandLen;
     kal_uint8*    SKRandPtr;
   ...
   { 
     ...
     kal_uint8     BakIDLen;
     kal_uint8*    BakIDPtr;
     kal_uint8     BakExpireLen;
     kal_uint8*    BakExpirePtr;
     kal_uint8     TKRandLen;
     kal_uint8*    TKRandPtr;
     kal_uint8     EncryptedBAK[16]; /* must be 16 according to 4.9 of C.S0023-D */
   }UpdateBAK;
   ...
   struct
   {
     ...
     kal_uint8     SKRandLen;
     kal_uint8*    SKRandPtr;
     kal_uint8     PacketIndexLen;
     kal_uint8*    PacketIndexPtr;
   }RetrieveSRTPSK;
   ...
   struct
   {
     ...
     kal_uint8     BakIDLen;
     kal_uint8*    BakIDPtr;   
     kal_uint8     TimeStampLen;
     kal_uint8*    TimeStampPtr;     
   }GenAuthSig;
...
typedef   struct 
{
    ...
    kal_uint16       RealmLen;
    kal_uint8*       Realm;
    kal_uint16       ServerNonceLen;
    kal_uint8*       ServerNonce;
    kal_uint16       ClientNonceLen;
    kal_uint8*       ClientNonce;
}   UimApplicationAuthentionMsgT;

typedef   struct 
{
    UimRspInfoT  UimRspInfo;
    kal_uint8        MAC_I[4];
}   UimUmacGenerationMsgT;
...
typedef  struct
{
    ...
    kal_uint8           SF_EUIMID[NAM_MEID_SIZE];
...
typedef   struct 
{
    ...
    kal_uint8           SmsNullRec[CUIM_BUFSIZE]; /*pointer to record status*/
...
typedef   struct 
{
    ...
    kal_uint8           Status;
    kal_uint8           MsgLen;
    kal_uint8           MsgData[CUIM_BUFSIZE];     
}   UimGetSmsRecRspMsgT;
...
typedef  struct {
   ...
   kal_uint8               NAIEntryId;
   kal_uint8               ChallengeLen;
   kal_uint8               pChallengeData[CUIM_BUFSIZE];
}   Uim3GSipChapReqEtsMsgT;
...
typedef  struct {
   ...
   kal_uint8            NAIEntryId; 
   kal_uint16           RRQLen;
   kal_uint8            pRRQData[CUIM_BUFSIZE];
}   Uim3GMipMNHAAuthReqEtsMsgT;
...
typedef  struct {
   UimRspInfoT       UimRspInfo;
   kal_uint16            PreRRQLen;
   kal_uint8            pPreRRQData[CUIM_BUFSIZE];
}   Uim3GMipRRQHashReqEtsMsgT;
...
typedef  struct {
   ...
   kal_uint8           RandLen;
   kal_uint8           AutnLen;      /* Length of AUTN (L1) */ 
   kal_uint8           Rand[16];    /*RANDA*/
   kal_uint8           Autn[16];         /* AUTN*/   
}   Uim3GAkaAuthReqEtsMsgT;
...
typedef struct {
   UimRspInfoT     UimRspInfo;     /* Requesting task's id, mailbox, msg ID */ 
} Uim3GPDIMSIGetMsgT;
...
#ifdef MTK_CBP
typedef struct {
   kal_bool  isEfPlRead;                 /* indicates if MF/2F05 already read */
   kal_uint8 UimEfPlLen;
   kal_uint8 UimEfPlData[MAX_EFPL_SIZE]; /*MF/2F05, prefer language file content */
} UimEfPlInfoT;
...
typedef struct {
   UimRspInfoT     UimRspInfo;
   kal_uint8 Index;    /*record number to be read*/
} UimIsimImpuReadMsgT;
...
extern void UimSetAuthCmdRspAllow(kal_bool value);
...
typedef enum
{
   ...
   DBM_CLEAR_CURR_CHV_ICCID                     /* clear first Element of DbmChvIccid[UICC_DBM_CACHE_CARD_NUM] */
...
typedef  struct
{
   UimRspInfoT UimRspInfo;
   kal_uint8       RecordIndex;
}  UimGetSimSmsScAddrMsgT;

typedef   struct 
{
    kal_bool      Ack;  /* success for KAL_TRUE; failure for KAL_FALSE */
    kal_uint8     Length; /* SC address length */
    kal_uint8     Tosca;  /* SC address type, 0x91:international; 0x81:Unkonw; 0xA1:National */
    char      Address[UIM_SIM_SC_ADDRESS_LENGTH];  /* SC address,in BCD code */
}  UimGetSimSmsScAddrRspMsgT; 
...
typedef  struct
{
   ...
   kal_uint8       MdnLen;
   kal_uint8       Mdn[MAX_NUMBER_BCD_LEN+1]; /* First byte is TON|NPI. */  
}  UimUpdateSimMdnRecMsgT;

typedef  struct
{
   kal_bool   Ack;
   kal_uint8  RecordIndex;
}  UimUpdateSimMdnRecRspMsgT;
...
typedef  struct
{
   kal_uint8 Status;
   kal_uint8 SstGprs;
   kal_uint8 SstPlmnwact;
   kal_uint8 SstOplmnwact;
   kal_uint8 SstHplmnact;
   kal_uint8 SstSMSPPDataDown;
   kal_uint8 SstSMSCBDataDown;
}  UimSimSessReqRspMsgT;
...
extern kal_uint8           Csim_AID[16], Usim_AID[16],Isim_AID[16];
extern UimCardTypeT UimGetCardType(void);
extern void UimSetCardType(UimCardTypeT type);
extern kal_uint8 UimGetCSTValueByIndex(UimECstIndexT index);
extern UimCarrierT UimGetCardCaiier( void );
#ifdef MTK_CBP
extern kal_uint16 UimReadEfPl(void);
extern kal_bool UimIsTestCard(void);
#endif
...
#ifdef MTK_DEV_C2K_IRAT
extern kal_bool UimGetCardIvsrStatus(void);
extern void UimStopAllTimer(void);
#endif

#if defined(SYS_OPTION_REMOTE_UICC) && !defined(MTK_DEV_C2K_IRAT)
extern IopDataChRetStatus (*pfunc_IopReadRemoteUicc)(void ** Buff, kal_uint16 * datalen, kal_uint32 flags);
extern IopDataChRetStatus (*pfunc_IopWriteRemoteUicc)(void * DataP, kal_uint16 DataLen, kal_uint32 flags);
#endif
...
extern kal_bool _UiccDbmIsUimModeDisabled(const char *Filename, const char *FunctionName, unsigned Linenumber);
