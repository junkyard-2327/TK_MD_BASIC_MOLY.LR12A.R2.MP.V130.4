...
//   ReportGapService,  /* gapservice will be decided by L1A */
   ...
   ReportTxComp,
#endif
//#ifdef __IS_2G_DYNAMIC_DISPLAY_MIPI_CLOCK_SUPPORT__
   ReportDynMIPIClk,
//#endif /*end of __IS_2G_DYNAMIC_DISPLAY_MIPI_CLOCK_SUPPORT__ */
   ReportModeChangeCNF,
   ReportFCChSRVCC,
#ifdef __GL1_GEMINI__
...
typedef struct
{
   ...
   int16 avgSnr;           /*Results for average SNR*/
#if IS_2G_RXD_SUPPORT
   MonitorPowerItem subPrx;  /* @field Results for sub channel blocks on primary antenna. */
   MonitorPowerItem subDrx;  /* @field Results for sub channel blocks on diversity antenna. */
...
typedef struct
{
   ...
   uint8             cipherKey[ MPH_CIPHER_KEY_LENGTH ];     /* @field Cipher key . Valid only if ciphering is enabled. */
} DChParams;
...
typedef struct
{
   ...
   Power          power_rxd[4];      /*RXD*//* @field (B) Power for bursts in dBm */
   intx           powerCount_rxd;    /*RXD*//* @field (C) Number of burst in power[] */
   bool           isPowerSwapped;    /* for L1C use; to indicate if power and powerDrx are swapped */
#endif
...
#if IS_2G_TAS_SUPPORT || IS_2G_RXD_SUPPORT
...
#if IS_2G_RXD_SUPPORT
   intx              preSnrSum;     /*@field (C) Sum of PreSNR of one block */
   uint8             preSnrCount;   /*@field (C) count of PreSNR of one block */
   uint8             RasObbCount;   /*@field (C) count of OBB of one block */
#endif

#if IS_2G_TAS_SUPPORT || IS_2G_RXD_SUPPORT
   TransmitAntenna   antenna;      /*@field (C) L1D report used Antenna info */
#endif
...
{
#if defined(__2G_RX_DIVERSITY_PATH_SUPPORT__)  
   L1D_RX_POWER ( *predict )( AGCData* p, ARFCN arfcn, TimeSlot timeSlot );
   void         ( *update )( AGCData* p, ARFCN arfcn, TimeSlot timeSlot, L1D_RX_POWER antennaPower, L1D_RX_RANGE_ERROR rangeError );
#else
...
#if defined(__2G_RX_DIVERSITY_PATH_SUPPORT__)   
   Power* bcchPower_DRX;   /* location to store power */
#endif
} AGCCellData;
...
typedef struct
{
   ...
   #if defined(__2G_RX_DIVERSITY_PATH_SUPPORT__)
   Power            interferencePower_DRX;
   #endif
#endif
   AGCDataDTX       dtx[2];
   intx             nextDTX;
#if defined(__2G_RX_DIVERSITY_PATH_SUPPORT__)
   AGCDataDTX       dtx_DRX[2];
   intx             nextDTX_DRX;
#endif
   Power            gainoffset;
...
struct AGCDataStruct
{
   ...
   AGCDataDTX*         dtx;                                   //AGCDataDTX struct is applying DRX
...
#if defined(__2G_RX_DIVERSITY_PATH_SUPPORT__)
   Power      bcchPower_DRX[ARFCN_COUNT];
#endif
} AGCPowerDB;
...
typedef enum
{
   ...
   ConflictWithLTE_NBCCh      = 15,
   ConflictWithLTE_PCh        = 16,
   ConflictWithLTE_Others     = 17
} ConflictType;
#endif
...
/*************************** for CT1 to queue function to SISR begin *****************************/
typedef enum
{
   GL1C_SISRQ_StopPchext,
   GL1C_SISRQ_StopPPchext,
   GL1C_SISRQ_ServingSchStop,
   GL1C_SISRQ_QueueServingSchStart,
   GL1C_SISRQ_FCChWinOffStart,
   GL1C_SISRQ_ChannelsUnlock,
   GL1C_SISRQ_Assign_3G_GAP,                    /* from L1SM_Assign_3G_Gap() */
#ifdef __GL1_GEMINI__
   GL1C_SISRQ_QueueReportSCh,
   GL1C_SISRQ_QueueReportFCCh,
#endif
   GL1C_SISRQ_FUNC_TYPE_NUM
} GL1C_SISRQ_FUNC_TYPE;

/*************************** for CT1 to queue function to SISR end   *****************************/
