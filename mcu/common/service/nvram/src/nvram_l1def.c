...
void L1D_CustomDynamicGetParam(kal_uint16 setIdx)
{
   ...
   for(band= 0; band < FrequencyBandCount; band++)
   {
      ...
      if (L1D_Cal_Data[setIdx].adjust_tpo_data.Adjust_TPO_GMSK[band] != 0)
      {
          kal_mem_cpy(Adjust_TPO_GMSK[band],L1D_Cal_Data[setIdx].adjust_tpo_data.Adjust_TPO_GMSK[band],(sizeof(nsft_adjust_tpo_t)));
      }
         #if defined(__EPSK_TX__)
            #if defined(__EPSK_ADJUST_TPO_SUPPORT__)
      if(L1D_Cal_Data[setIdx].adjust_tpo_data.Adjust_TPO_EPSK[band]!=0)
      {
         kal_mem_cpy(Adjust_TPO_EPSK[band],L1D_Cal_Data[setIdx].adjust_tpo_data.Adjust_TPO_EPSK[band],(sizeof(nsft_adjust_tpo_t)));
      }
