...
typedef struct _ex_overview_t
{
    ...
    volatile EX_STEP_T   ex_steplog[TOTAL_VPE_COUNT];  // 32 = 8*4
    kal_uint32           ect_status;                   // 4 //offending vpe
    kal_uint8            usip_scq_offending_core;      // 1  usip+scq
    kal_uint8            mcu_exception_count[TOTAL_VPE_COUNT]; // 4
    kal_uint8            pad[47];                      // 47 = 512-(4+4+96+284+4+32+4+1+4+32)
...
#define EX_DOR_EXCP_AREA_SIZE   ( TOTAL_VPE_COUNT * 8 * sizeof(kal_uint32))
...
typedef struct _ex_mcu_sysinfo_t
{
    ...
    kal_uint32              dor_excep_area[EX_DOR_EXCP_AREA_SIZE/sizeof(kal_uint32)];
