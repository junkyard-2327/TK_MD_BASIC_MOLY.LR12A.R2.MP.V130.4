...
#****************************************************************************
my $VERNO     = " a0.05";
             # a0.05_R2,2017/12/13,  Provide DRDI usage info in CMD_GetRAMUsage
             # a0.04_R2,2017/12/06, Carl, Add GetMDRealUsedEnd() for saving DSP space
...
sub CMD_GetRAMUsage
{
	...
	if (hex($strERsize)) {
	  ...
	  if ($append == 1) {
	    ...
	    push @UsedRAMAreas, [$nERaddr, $ERsize]
	  }
	}
      }
	...
	$FRindex +=1;
	$totalRAM += $refUsed->[1];
	PrintLog($PrintFunc, "RAM Usage $FRindex  ".CommonUtil::Dec2Hex($refUsed->[1])." = " .GetSizeFromBytes($refUsed->[1])."\tfrom ". CommonUtil::Dec2Hex($refUsed->[0])." to ".CommonUtil::Dec2Hex($refUsed->[0]+$refUsed->[1])."\n");
      }
