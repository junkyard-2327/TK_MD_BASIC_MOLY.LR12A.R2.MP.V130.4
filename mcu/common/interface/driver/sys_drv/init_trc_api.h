...
#define INT_TRC_C(index) \
    do { \
        ...
        if (glb_vpe_idx == 0) { \
            *((kal_uint32 *)g_EMM_BOOTTRC_VPE0_ADDR)= index; \
        } \
        if (glb_vpe_idx == 1) { \
            *((kal_uint32 *)g_EMM_BOOTTRC_VPE1_ADDR)= index; \
        } \
        if (glb_vpe_idx == 2) { \
            *((kal_uint32 *)g_EMM_BOOTTRC_VPE2_ADDR)= index; \
        } \
        if (glb_vpe_idx == 3) { \
            *((kal_uint32 *)g_EMM_BOOTTRC_VPE3_ADDR)= index; \
        } \
        INC_Init_Step[glb_vpe_idx].HS1_boot_trace = index; \
        INC_Init_Step[glb_vpe_idx].HS1_boot_trace_time = ust_get_current_time(); \
    } while (0)
/*
