...
} nvram_lte_unavail_sys_struct;

/* Structure for item in LTE Available File.
*/
typedef struct{
  nvram_lte_unavail_sys_struct sys;   /* 1x system information */
  kal_uint8  lte_unavail_count;  /* fail counter for LTE unavail */  
  kal_uint8  lte_avail_count;  /* fail counter for LTE avail */
} nvram_lte_unavail_item_struct;
...
typedef struct
{
    ...
    kal_uint8  lte_unavail_db_num;
    nvram_lte_unavail_item_struct lte_unavail_db[ GMSS_NV_ITEM_LTE_AVAIL_MAX ];
