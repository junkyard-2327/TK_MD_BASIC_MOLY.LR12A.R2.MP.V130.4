...
typedef struct kal_workqueue_node {
 ...
 //debug+
    ...
    kal_uint32           insert_tc;           
    kal_uint32           dequeue_tc;
    kal_uint32           reserved;
#endif /*MT_FLOW_DEBUG*/
...
#if defined(MT_FLOW_DEBUG)
#define HRT_CQUEUE_INSERT(_q, _item, idx, tc)                  \
do{                                                                  \
    ...
    (_q).num_of_items ++;                                            \
    (_q).queue[idx].insert_tc |= 1 << tc; \
    (_q).queue[idx].insert_timestamp = ust_get_current_time(); \
}while(0)

#define HRT_CQUEUE_DISCARD(_q, idx, tc)                             \
do{                                                                  \
    idx = (_q).head; \
    (_q).head++;                                                     \
...
#define LISR_MT_QUEUE_NUMBER 3          // 0: common workqueue, 1: TC2 workqueue, 2: TC3 workqueue
...
{
    kal_uint32 magic_id;
    kal_workqueue_struct  *wq[LISR_MT_QUEUE_NUMBER];
} kal_lisr_mt;
