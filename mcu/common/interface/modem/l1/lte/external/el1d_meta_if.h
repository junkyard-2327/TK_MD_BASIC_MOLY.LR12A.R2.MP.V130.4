 ...
 *
 * 02 08 2018 wade.chen
 * [MOLY00306800] B28??工具控制±0.3dB
 * 	
 * 	[EL1C] NSFT tx power offset
 *
 * 02 06 2018 wade.chen
 * [MOLY00305372] [  ?急] B3+B7 CA校准??不?，疑似patch引起
 * 	
 * 	Fix  LTE NSFT pseudo TRX in force mode tx route bug
 *
 * 01 19 2018 wade.chen
 * [MOLY00302842] LTE NSFT ??RX的?候，需要打?TX
 ...
 * [93][R2][EL1D DPD] mipi/TPC/custom code update.
 *
 * 08 02 2017 ming-chien.hung
 * [MOLY00268393][EL1D DPD] mipi/TPC/custom code update.
 *
 * 07 27 2017 yu-hsuan.sung
 * [MOLY00266857] [GEN93][LTE] RX measurement delay for DFE under plan B scenario
 * .
 *
 * 07 26 2017 yungshian.lai
 * [MOLY00266063] [EL1TST] GPS coclock v2.0 Development - 4G Read AuxADC /2G add capability and modify DHL PSTrace - LR12A.R2.MP.
 *
...
#define ERF_MAX_LTE_ET_SET_DATA_NUM_V3              (40)  /* floor(MMRF_MAX_TOOL_PEER_SIZE_PER_CMD / sizeof(MMRfTestCmd_SetLteEtFacCalAll_ReqPdu)) */
#define ERF_MAX_LTE_ET_GET_DATA_NUM_V3              (ERF_MAX_LTE_ET_SET_DATA_NUM_V3)
#define ERF_MAX_ET_NUM_ROUTE                     (LTE_MAX_TX_COMP_ROUTE_NUM + LTE_FILTER_MAX_SUPPORT_BAND_NUM + LTE_TARGET_MAX_SUPPORT_BAND_NUM + LTE_FILTER_MAX_SUPPORT_BAND_NUM)
#define ERF_MAX_ET_NUM_ROUTE_TYPE                (2)   // 0: ROUTE = 301, 1: ROUTE = 522 2: 93 ROUTE = 85
 ...
 ******************************************************************************/
/* Calibration success/fail for tool reference */
typedef enum
{
   ERF_TEST_ET_CAL_START_ERROR      = 0,
   ERF_TEST_ET_CAL_START_SUCCESS    = 1

} ERfTest_EtFacCal_StatusType;
...
typedef struct
{
   ...
   /*93ET remove LUT1*/
   kal_int16  GainComp_dB_0[ERF_MAX_ET_NUM_SUBBAND_V3];
   kal_int16  Tx_gain_idx[ERF_MAX_ET_NUM_SUBBAND_V3];    /*93 renamed*/
   kal_int16  GrpDelayoffset_0[ERF_MAX_ET_NUM_SUBBAND_V3][ERF_MAX_ET_NUM_RB_IDX_V3];
   kal_int16  cca_poutgain[ERF_MAX_ET_NUM_SUBBAND_V3];
   kal_int16  ETK_algorithm;   /*1 for normal, 2 for smooth LUT*/

   /*93ET increase for ETK step2 */
   kal_uint16 duration_of_gainK;
   kal_uint16 Ite_num;
   kal_uint16 duration_dlyK;
   kal_int16  step2_define_cal_dly[ERF_MAX_ET_STEP2_DEFINE_CBW_V3][ERF_MAX_ET_STEP2_DEFINE_NUM_DELAY_V3];
   kal_uint16 step2_define_cal_cbw[ERF_MAX_ET_NUM_SUBBAND_V3];

} ERfTestCmd_StartETFacCal_peer;

typedef struct
{
   kal_uint16  route_num;    /* The number of route in this start command */   
   kal_uint16  cal_band[LTE_TARGET_MAX_SUPPORT_BAND_NUM];   /*Use cfg to decide ET calibration band*/
   kal_uint16  cmd_set_done;  /* last start command indicator. 0:No, 1:Yes, 2:PA_gain K 3:GC_delay search Lab K*/
   
}ERfTestCmd_StartEtFacCal_ReqInfo;
...
}ERfTestCmd_StartEtFacCal_CnfInfo;
/*******************************************************************************
 * MMRF_TEST_CMD_SET_ET_CAL_RESULT_V3
 ******************************************************************************/
typedef enum
{
   ERF_TEST_SET_ET_COARSE_INVALID         = 0,
   ERF_TEST_SET_ET_COARSE_OK              = 1,
   ERF_TEST_SET_ET_COARSE_OFFSET_MISMATCH = 2  /* count/offset mismatch */

}ERfTest_SetEtCalResult_CoarseStatus;
...
}ERfTest_SetEtCalResult_RouteStatus;
...
{   
   kal_uint16                           route_num; 
   ERfTest_SetEtCalResult_CoarseStatus  coarse_status;
   ERfTest_SetEtCalResult_RouteStatus   route_status[ERF_MAX_LTE_ET_SET_DATA_NUM_V3];

}ERfTestCmd_SetEtCalResult_CnfInfo;
...
typedef struct
{
   ...
   kal_uint16 VinVpaLut[ERF_MAX_ET_NUM_SUBBAND_V3][ERF_MAX_ET_LUT_PWR_NUM_V3]; /* by subband, by number of Pin */
   kal_int8   VinPmLut [ERF_MAX_ET_NUM_SUBBAND_V3][ERF_MAX_ET_LUT_PWR_NUM_V3]; /* by subband, by number of Pin */
   kal_uint8  VinAmLut [ERF_MAX_ET_NUM_SUBBAND_V3][ERF_MAX_ET_LUT_PWR_NUM_V3]; /* by subband, by number of Pin */
   kal_int16  PathDelay[ERF_MAX_ET_NUM_SUBBAND_V3][ERF_MAX_ET_NUM_CBW_V3];     /* by subband, by number of CBWs */
   kal_int16  Pout_max [ERF_MAX_ET_NUM_SUBBAND_V3];
   kal_int16  Pout_gain_offset[ERF_MAX_ET_NUM_SUBBAND_V3];
   kal_int16  GainComp_dB[ERF_MAX_ET_NUM_SUBBAND_V3];
   kal_int16  GainComp_dB_0[ERF_MAX_ET_NUM_SUBBAND_V3];
   kal_int16  GrpDlyOfstBySubband  [ERF_MAX_ET_NUM_SUBBAND_V3][ERF_MAX_ET_NUM_RB_IDX_V3];
   kal_int16  Tx_gain_idx[ERF_MAX_ET_NUM_SUBBAND_V3];     //93 renamed
   kal_int16  PaGainFineTuneStep1[ERF_MAX_ET_NUM_SUBBAND_V3];
   kal_int16  PaGainFineTuneStep2[ERF_MAX_ET_NUM_SUBBAND_V3][ERF_MAX_ET_NUM_CBW_V3];
   kal_uint32 EtCalDoneFlag;
   kal_uint32 EtCalhpueDoneFlag;
   kal_int16  cca_poutgain[ERF_MAX_ET_NUM_SUBBAND_V3];
   kal_int16  ETK_algorithm;                                                                                        /*1 for normal, 2 for smooth LUT*/
...
{
   kal_uint16           route_num;
   ERfTest_EtRouteInfo  route_info[ERF_MAX_LTE_ET_GET_DATA_NUM_V3];
   
}ERfTestCmd_GetEtCalResult_ReqInfo;
...
typedef struct
{
   kal_uint32 SwOptMode       [ ERF_MAX_ET_NUM_ROUTE ];                                /* 0:invalid 1:ET mode 2:APT mode*/
   kal_uint16 Band            [ ERF_MAX_ET_NUM_ROUTE ];                                /*Band*/
   kal_uint16 Route           [ ERF_MAX_ET_NUM_ROUTE ];                                /*Comp_route*/
   kal_uint16 path_select     [ ERF_MAX_ET_NUM_ROUTE ];                                /*bit0 :filter bit1 : CCA*/
   kal_uint16 SubbandFreq     [ ERF_MAX_ET_NUM_ROUTE ][ ERF_MAX_ET_NUM_SUBBAND_V3 ];     /*subband frequency*/
}ERfTestCmd_CheckEtPathDelaySearch_peer;
...
typedef union 
{
   ERfForceEtSetting  force_et_setting;
}ERfEtSetting;
