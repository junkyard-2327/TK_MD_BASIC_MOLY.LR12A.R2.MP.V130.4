...
ifdef TEST_LOAD_TYPE
  ifeq ($(filter BASIC,$(TEST_LOAD_TYPE)),)
    # COMPLIST       += tl1_pcore
    COMPLIST       += tl1 ll1 
    # DM (EGPRS + 3G TD) or DM (GPRS + 3G TD)
  endif
endif
...
ifeq ($(filter L1S BASIC,$(TEST_LOAD_TYPE)),)
  ...
  ifeq ($(strip $(GERAN_MODE_SUPPORT)),NONE)
    ...
    COMPLIST += md_drv mm gas_fdd gas_smp gas_tdd gas_smp_tdd ratcm 
  endif
  ...
  ifneq ($(strip $(UTRAN_MODE_SUPPORT)),NONE)
    COMPLIST += ratdm ratdm_sec uas_fdd uas_tdd fsm sndcp sm llc asinterfaces pam
