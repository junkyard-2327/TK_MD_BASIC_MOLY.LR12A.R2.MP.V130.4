...
ifneq ($(call Upper,$(strip $(EUTRAN_MODE_SUPPORT))),NONE)
  ...
  ifneq ($(strip $(LTE_DUMMY_PROTOCOL_ENABLE)),TRUE)
    ...
    LTE_SEC_COMP      = emm esm eval errc etc
    ifneq ($(wildcard make/module/modem/lte_sec/emm_sec.mak),)
      LTE_SEC_COMP += emm_sec
    else 
      ifneq ($(wildcard $(strip $(CUS_MTK_LIB))/libemm_sec.a),)
        LTE_SEC_COMP += emm_sec
      endif
      ifneq ($(wildcard $(strip $(CUS_MTK_REL))/UESim_lib/emm_sec.lib),)
        LTE_SEC_COMP += emm_sec
      endif
      ifneq ($(wildcard $(strip $(CUS_MTK_REL))/MoDIS_lib/emm_sec.lib),)
        LTE_SEC_COMP += emm_sec
      endif        
    endif
