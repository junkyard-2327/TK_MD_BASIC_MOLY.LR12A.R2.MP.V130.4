...
# generate multiple catcher filter and dsp filter files
  ifeq ($(strip $(CUSTOM_RELEASE)),TRUE)
    ifeq ($(strip $(MTK_SUBSIDIARY)),TRUE)
	@perl $(COMMON_ROOT)/tools/multi_catcher_filter.pl $(strip $(PROJDIR)) $(strip $(CUS_MTK_REL)) $(POSTGEN_CFG_MODEM) $(POSTGEN) $(CODEGEN_DATABASE_OUT) $(CUSTOMIZATION) $(FILTER_INI) $(strip $(DSP_PROJECT)) $(strip $(DSP_FLAVOR)) TRUE $(LOG2SD_CATCHER_FILTERS_FILE_LIST)
    else
	@perl $(COMMON_ROOT)/tools/multi_catcher_filter.pl $(strip $(PROJDIR)) $(strip $(CUS_MTK_REL)) $(POSTGEN_CFG_MODEM) $(POSTGEN) $(CODEGEN_DATABASE_OUT) $(CUSTOMIZATION) $(FILTER_INI) $(strip $(DSP_PROJECT)) $(strip $(DSP_FLAVOR)) FALSE $(LOG2SD_CATCHER_FILTERS_FILE_LIST)
    endif
  else
	@perl $(COMMON_ROOT)/tools/multi_catcher_filter.pl $(strip $(PROJDIR)) $(strip $(CUS_MTK_REL)) $(POSTGEN_CFG_MODEM) $(POSTGEN) $(CODEGEN_DATABASE_OUT) $(CUSTOMIZATION) $(FILTER_INI) $(strip $(DSP_PROJECT)) $(strip $(DSP_FLAVOR)) TRUE $(LOG2SD_CATCHER_FILTERS_FILE_LIST)
  endif
