...
ifeq ($(strip $(MBIS_EN)),TRUE)
	...
	@perl $(strip ${PCORE_TOOLS})/auto_adjust_mem.pl 1 $(TMPDIR)/~auto_adjust_mem_1.log > $(strip $(COMPLOGDIR))/auto_adjust_mem.log $(WITH) \
		($(IF_ERR_TRUE) $(THEN) \
			($(MAKE) -f$(strip $(MAKE_COMMON))/build.mak -r -R update_memconfig) $(WITH) \
...
#=== SBC
	...
	@$(IF_EXIST) $(strip $(TARGDIR))/$(strip $(BIN_FILE)) $(THEN)\
		(perl $(strip ${PCORE_TOOLS})/gfh_process.pl $(strip $(TARGDIR))/$(strip $(BIN_FILE)) $(strip $(TMPDIR))/~gfh_cfg.tmp "$(strip $(PROJECT_MAKEFILE))" $(strip $(COMPLOGDIR))/gfh_process.log) > $(strip $(COMPLOGDIR))/gfh_process.log 2>&1 $(WITH) \
