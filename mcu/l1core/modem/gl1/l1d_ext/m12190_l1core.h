...
#include "mmrf_common_cid.h"  //IS_MML1_PMIC_MT6356
#if IS_2G_RX_DIVERSITY_PATH_SUPPORT
#include "l1tst_public.h"
#include "l1tst_ft_msg_struct.h"  //Add for using L1TST Struct,example: RXD Get info - RfTestCmdCalInfoV5_T
#endif
/*...................................*/
...
typedef enum
{  
   L1D_ELNA_IN_PATH_OFF,      //for RXD off only                       
   L1D_ELNA_UNUSED,                     
   L1D_ELNA_ALYWAYS_ON,      
   L1D_ELNA_CAN_SWITCH,    //narrow band matching       
   L1D_ELNA_CAN_SWITCH_WB, //wide band matching 
} L1D_ELNA_USAGE;
   #else
typedef enum
{                               
   ...
   #endif
#endif
#define US_TO_QB_ROUNDUP(us)  ((((us)*1110)>>10)+1) /*multiply 1110/1024(=1.08398) instead of 13/12(=1.08333)*/
/*...................................*/
...
#define MIPI_SETDATA_ELNA          0x04
#define MIPI_SETDATA_ELNA_RXD      0x05
#endif
...
typedef  struct
{
   ...
   short bySlotMod[4];      /* modulation type of every TX slot in 1 frame */
   short bySlotPower[4];    /* power of every TX slot in 1 frame */
   short bySlotDAC[4];      /* GMSK DAC of every TX slot in 1 frame */
   short EMmaxDAC;          /* MAX GMSK DAC among slots in 1 frame*/
   short EMmaxPower;        /* MAX power among slots in 1 frame*/
   short EMmodType;         /* modulation type of MAX power slot */
#endif
#if IS_TX_POWER_CONTROL_SUPPORT
   TXPC_MODE      txpc_mode;
   TXPC_TYPE      txpc_type;
...
char           is_tx_decrease_power_needed_tas;
   ...
   uint8          tas_ant_sel;
   uint8          tas_ant_sel_CatA;   //To swap control of DPDT for ARX+TAS coexistence
   uint8          tas_ant_sel_RXTX;   //For RX/TX TAS debug trace
   bool           isNormalSIM;
   #if IS_2G_TAS_ANT_IDX_FOR_PM_WINDOW_SUPPORT
   unsigned char  is_TAS_PMWindow;
   #endif
#endif
   ...
   unsigned char  is_PMWindow;
#endif
#if IS_L1D_MMRF_BSIBPIRS_DYNAMIC_ALLOCATION_SUPPORT
   char           mmrf_init_flags;
#endif
#if IS_2G_EXTERNAL_LNA_SUPPORT
   MML1_FE_ELNA_IND_T *elna_database[5];
   kal_int32           elna_settling_time[5];
   MMRF_ELNA_TYPE_E    xwin_elna_status;
   #if IS_2G_RX_DIVERSITY_PATH_SUPPORT
   MML1_FE_ELNA_IND_T *elna_database_DRX[5];
   kal_int32           elna_settling_time_DRX[5];
   MMRF_ELNA_TYPE_E    xwin_elna_status_DRX;
   #endif
#endif
#if IS_GL1D_RXD_SUPPORT
   L1D_RXD_MODE        rxd_mode; //Use in BSI, BPI, MIPI. (excluding AGC!)
#endif 
}  sRFSETTING;
...
#define  CLR_FLAG( flag, mask )           ((flag)&=~(mask))
#define  CHECK_FLAG( flag, mask )         ((flag)&(mask))

/*============================================================================*/

#if IS_L1D_MMRF_BSIBPIRS_DYNAMIC_ALLOCATION_SUPPORT
#define  RF_SET_FLAG( flag, mask )           ((flag)|=(mask))
#define  RF_CLR_FLAG( flag, mask )           ((flag)&=~(mask))
#define  RF_CHECK_FLAG( flag, mask )         ((flag)&(mask))

/* define flags for l1d_rf.mmrf_init_flags ............................*/
#define  MMRF_BSI_INIT_BIAS           0x0001
#define  MMRF_BPI_INIT_BIAS           0x0002

/* define flags for l1d_mipi.mmrf_init_flags ............................*/
#define  MMRF_MIPI_INIT_BIAS          0x0004
#endif 
...
{
   int            arfcn;
#if IS_EPSK_TX_SUPPORT || IS_RF_MT6162 || IS_RF_MT6163 || IS_RF_MT6280RF || IS_RF_MT6169 || IS_RF_MT6166 || IS_RF_MT6165 || IS_RF_MT6176 || IS_RF_MT6179 || IS_RF_MT6177L || IS_RF_MT6177M
   ...
   MMRFC_GSM_RESULT_T rfc_data;             /* 2G RF POC Result */
   signed short   pga_ab_switch_pwr_idx[4]; /* power_idx that use pga_b */
   signed short   pga_a_inl[4][MMRFC_GSM_TX_DNL_PGA_A_GAIN_STEPS+MMRFC_GSM_TX_DNL_PGA_AUX_GAIN_STEPS]; 
#endif
#if IS_DCS_NB_WB_SWITCH_SUPPORT
...
typedef  struct
{
   ...
   Gain  blk_gain;
   long  blk_setting;
   L1D_LNA_TYPE_E blk_lna;
   MMRF_ELNA_TYPE_E elna_type;
} sL1DAGCHALSETTING;
...
typedef  struct
{
   const Gain  max_request_gain;
   const Gain  min_request_gain;
   const Power  min_predict_power;
   const Power  max_predict_power;
   const Power  ibbd_threshold;
   const Power  hrd_threshold;
   const sL1DAGCHALSETTING table[];
} sL1DAGCHALDATA;
#endif
/*...................................*/
#if IS_2G_EXTERNAL_LNA_SUPPORT
...
extern BPI_DATA_SIZE pdata_gmsk;
extern BPI_DATA_SIZE pdata_8psk;
#else
extern BPI_DATA_SIZE pdata_gmsk;
extern BPI_DATA_SIZE pdata_8psk;
#endif
...
typedef  enum
{
   ...
   DRFIQ0,
#if IS_RF_MT6177M
/*77M only support PRX0 and DRX0*/
#else
   PRFIQ1,
   DRFIQ1,
#endif
...
#if IS_RF_MT6256RF || IS_RF_MT6251RF || IS_RF_MT6255RF || IS_RF_MT6280RF || IS_RF_MT6169 || IS_RF_MT6166 || IS_RF_MT6165 || IS_RF_MT6176 || IS_RF_MT6179 || IS_RF_MT6177L || IS_RF_MT6177M
...
#if IS_RF_MT6280RF || IS_RF_MT6169 || IS_RF_MT6166 || IS_RF_MT6176 || IS_RF_MT6179 || IS_RF_MT6177L || IS_RF_MT6177M
...
#if IS_RF_MT6166 || IS_RF_MT6176 || IS_RF_MT6179 || IS_RF_MT6177L || IS_RF_MT6177M
void L1D_RF_SplitRxPLLSetting(unsigned long rfN, unsigned long ifN, unsigned short *Nint, unsigned long *Nfrac);
void L1D_RF_AdjustRxPLLSetting(unsigned short Nint, unsigned long Nfrac, signed long NfracOffset, unsigned long * rfN, unsigned long * ifN);
void L1D_RF_SplitTxPLLSetting(unsigned long rfN, unsigned long ifN, unsigned short *Nint, unsigned long *Nfrac);
void L1D_RF_AdjustTxPLLSetting(unsigned short Nint, unsigned long Nfrac, signed long NfracOffset, unsigned long * rfN, unsigned long * ifN);
...
#if IS_2G_RX_DIVERSITY_PATH_SUPPORT
L1D_RX_GAIN   L1D_RF_GetGainSetting_HAL( int rf_band, int arfcn, L1D_RX_POWER predict_power, long *gain_settingPRX, long *gain_settingDRX );
#define  L1D_RF_GET_GAIN_SETTING( _real_gainPRX, _real_gainDRX, _band, _arfcn, _predict_power, _gain_settingPRX, _gain_settingDRX ) \
{  L1D_RX_GAIN     return_real_gain;                                                    \
   return_real_gain = L1D_RF_GetGainSetting_HAL(_band,_arfcn,_predict_power, _gain_settingPRX, _gain_settingDRX ); \
   _real_gainPRX += return_real_gain.PRX;                                                \
   _real_gainDRX += return_real_gain.DRX;                                                \
}
L1D_RX_POWER L1D_RF_GetGainSetting_INVERSE( int rf_band, int arfcn, L1D_RX_GAIN request_gain);
L1D_RX_POWER L1D_RF_CalibrationInputpowerCheck( int rf_band, int arfcn, L1D_RX_POWER instrument_downlink_power, LNACalSeqV5_E prx_lna_mode_in_TST, LNACalSeqV5_E drx_lna_mode_in_TST);
void L1D_RF_GetCalibrationINFO( RfTestCmdCalInfoV5AllBandCnfPdu_T *gl1tst_get_rxd_info );
...
unsigned long L1D_RF_Get_STX_CW511( int band );
unsigned long L1D_RF_Get_STX_CW529( unsigned long cw549, unsigned long cw550 );
kal_uint32 L1D_GetBandSupport_L1Core( void );
#elif IS_RF_MT6177L || IS_RF_MT6177M
void L1D_RF_Set_RFC_Result( MMRFC_GSM_RESULT_T *buff );
void L1D_RF_GetBPI_PAOnOff( int bandidx, unsigned long* POC_PAOn, unsigned long* POC_PAOff);
unsigned long L1D_RF_Get_STX_CW529( unsigned long cw549, unsigned long cw550 );
unsigned long L1D_RF_Get_TX_CW705_DET_PGA_CSEL( unsigned long cw787_tx_lpf_rc_code);
kal_uint32 L1D_GetBandSupport_L1Core( void );
#endif
...
/* For RX RSSI Temperature Compensation */
#define RSSI_TC_SLOPE_LB              40  // Unit: Degree/dB
#define RSSI_TC_SLOPE_HB              40  // Unit: Degree/dB
#define RSSI_TC_SLOPE_SCALE            3
...
/* For RX RSSI Temperature Compensation */
#define RSSI_TC_SLOPE_LB              48  // Unit: Degree/dB
#define RSSI_TC_SLOPE_HB              40  // Unit: Degree/dB
#define RSSI_TC_SLOPE_SCALE            3
...
/* For RX RSSI Temperature Compensation */
#define RSSI_TC_SLOPE_LB              56  // Unit: Degree/dB
#define RSSI_TC_SLOPE_HB              48  // Unit: Degree/dB
#define RSSI_TC_SLOPE_SCALE            3
...
extern unsigned short rfic_flags_for_scheduleread;
extern unsigned short band_info_for_dualrfic;
...
unsigned short  L1D_RF_FindGainStep( unsigned long gain_setting, FrequencyBand rf_band );
      #if IS_2G_RX_DIVERSITY_PATH_SUPPORT
unsigned short  L1D_RF_FindGainStep_RXD( unsigned long gain_setting, FrequencyBand rf_band );
      #endif
   #endif
#endif
...
void  L1D_RF_SetPData_FSIWinOff( unsigned short index_pr3, unsigned short index_pr3a );

#if IS_RF_MT6163
void  L1D_RF_SetSData_FSIWinOff_SR3( int idx );
#elif IS_RF_MT6177L || IS_RF_MT6177M
void  L1D_RF_SetSData_SR2_CT2( void );
void  L1D_RF_SetSData_FSIWinOff_SR3( unsigned short event_idx );
#endif

void  L1D_RF_SetPData_PR( void );
void  L1D_RF_SetPData_PT( void );
#if IS_2G_EXTERNAL_LNA_SUPPORT
void  L1D_RF_SetPData_PR2E( void );
void  L1D_RF_SetPData_PR2E_CT2( void );
#endif
...
void  L1D_MIPI_RX2E_WIN( signed int xwin_idx, unsigned short win_type, int32 regbias );
#endif
...
#define LPM_FREQ_DRIFT_PPM   (-15)
   #elif IS_CHIP_MT6292               //IS_MML1_CHIP_MT6292_AND_LATTER_VERSION
#define LPM_FREQ_DRIFT_PPM   (-7)
   #elif (IS_CHIP_MT6293 && IS_MML1_PMIC_MT6356)
#define LPM_FREQ_DRIFT_PPM   ((MML1_RF_DCXO_Internal_Offset_Cap_Enable())?19:(-4))  // fine tune 93+PMIC_6356
   #elif (IS_CHIP_MT6293 && IS_MML1_PMIC_MT6358)
#define LPM_FREQ_DRIFT_PPM   ((MML1_RF_DCXO_Internal_Offset_Cap_Enable())?9:(0))  // fine tune 93+PMIC_6358
   #elif IS_CHIP_MT6293
#define LPM_FREQ_DRIFT_PPM   ((MML1_RF_DCXO_Internal_Offset_Cap_Enable())?19:(-4))  //To avoid 93+other PMIC will build err, but we need to fine tune DRIFT for other PMIC
   #elif IS_CHIP_SER_AND_LATTER(CHIP_ID_MT6293) //l1d_cid.h  //#elsif IS_MML1_CHIP_MT6295_AND_LATTER_VERSION  //95 latter //mmrf_common_cid.h
#define LPM_FREQ_DRIFT_PPM   -4
   #else
...
unsigned short L1D_RF_Check_If_ExternalLNA( int rf_band );
      #if IS_2G_RX_DIVERSITY_PATH_SUPPORT
unsigned short L1D_RF_Check_If_ExternalLNA_RXD( int rf_band );
      #endif
unsigned short L1D_RF_GetSupportBitmap_ELNA( void );
      #if IS_2G_EXTERNAL_LNA_BYPASS_SUPPORT
unsigned short L1D_RF_GetBypassSupportBitmap_ELNA( void );
signed short L1D_RF_GetELNABypass_Gain_Threshold( void );  // unit of 1/8dB
...
unsigned int  L1D_RF_Check_If_Prx_Enable( int rf_band );
unsigned int  L1D_RF_Check_If_Drx_Enable( int rf_band );
   #endif
#endif /* IS_RF_MT6177L */
...
unsigned short L1D_RF_GetSupportBitmap_ELNA( void );
      #if IS_2G_EXTERNAL_LNA_BYPASS_SUPPORT
unsigned short L1D_RF_Check_If_ExternalLNA_BYPASS( int rf_band );
unsigned short L1D_RF_GetBypassSupportBitmap_ELNA( void );
signed short L1D_RF_GetELNABypass_Gain_Threshold( void );  // unit of 1/8dB
signed short L1D_RF_GetDSPSetPointELNABypasssLowMode_MaxPin( void );
      #endif
...
#if IS_L1D_MMRF_BSIBPIRS_DYNAMIC_ALLOCATION_SUPPORT
extern uint8   L1D_GetWinFrameCnt(void);
extern int32   L1D_GetWinBias(void);
extern uint32  L1D_GetOneBPITime(unsigned short index);
extern uint32  L1D_GetOneBSITime(unsigned short index);
extern void    L1D_ClearOneBPITime(unsigned short index);
extern void    L1D_ClearOneBSITime(unsigned short index);
...
#define IS_BSI_IMM_SEND_COMPLETE             (0x1)
   ...
   #if IS_L1D_MMRF_BSIBPIRS_DYNAMIC_ALLOCATION_SUPPORT
#define IS_BSI_IMM_SEND_COMPLETE             (0x1)
   #elif IS_CHIP_MT6280 || IS_CHIP_MT6583_MD1 || IS_CHIP_MT6583_MD2 || IS_CHIP_MT6572 || IS_CHIP_MT6290 || IS_CHIP_MT6595 || IS_CHIP_MT6752_MD1 || IS_CHIP_MT6752_MD2 || IS_CHIP_TK6291 || IS_CHIP_MT6755 || IS_CHIP_MT6292
...
void L1D_RF_SWTP_Trace_SAR_Table( uint32 v1, uint32 v2, int32 v3, int32 v4 );
...
/*===============================================================================*/
   #if (IS_2G_TAS_SUPPORT)
extern void L1D_UpdateTASParameters(L1D_CUSTOM_TAS_NVRAM_T* custom_param);
...
extern void L1D_Update_MIPICLOCK_GROUP( L1D_CUSTOM_DISPLAY_MIPI_CLK_NVRAM_T* GROUP_ARFCN);
//#endif
...
/*===============================================================================*/
/* 2G APC Select function                                                        */
/*===============================================================================*/
void L1D_RF_Set_Mixedsys_2G_APC_Select( unsigned short apc_2g_sel );

   #if IS_L1D_MMRF_BSIBPIRS_DYNAMIC_ALLOCATION_SUPPORT
void L1D_RF_init_BSIData_Start_Idx(void);
void L1D_RF_init_BPIData_Start_Idx(void);
void L1D_RF_init_MIPIData_Start_Idx(void);
   #endif
   #if IS_2G_RX_DIVERSITY_PATH_SUPPORT
extern void L1D_UpdateRASParameters(L1D_CUSTOM_RAS_NVRAM_T * ras_custom_params);
...
/*===============================================================================*/
/* 2G SAR TX POWER BACKOFF BY DB function                                        */
/*===============================================================================*/
